...
async function geminiAnalyzeScreenshot_(env,img,candidates,context={}){
  ...
  /* 긴 member_id를 모델에 직접 출력시키지 않고 짧은 후보키를 사용한다.
     JSON 생성이 중간에 깨지는 문제와 토큰 낭비를 동시에 줄인다. */
  ...
  const keyMap=new Map(keyed.map(x=>[x.key,x]));
  const roster=keyed.map(c=>`${c.key}\t${c.nickname}\t${c.server_name}\t${c.guild_name}`).join("\n");
...
매우 중요:
- API 호출은 지금 이 1회뿐입니다. 별도 호출을 요구하지 마세요.
- 최종 답을 내기 전에 이미지들을 내부적으로 3차례 반복 검토하세요. 검토 과정이나 생각은 출력하지 말고 최종 결과만 출력하세요.
  1차 검토: 화면 전체와 각 확대 조각에서 보이는 플레이어 닉네임을 넓게 탐색.
  2차 검토: 1차에서 놓쳤을 가능성이 있는 후보 명단을 다시 훑으며 중앙 밀집구역, 좌측, 우측, 상·하단을 재검사.
  3차 검토: 1·2차 결과를 다시 이미지와 대조하여 누락, 중복, 오독을 교정하고 최종 확정.
- 3차례 중 한 번이라도 글자가 명확히 보이면 최종 후보에 포함할 수 있지만, 후보 명단에 있다는 이유만으로 추측해서 넣으면 안 됩니다.

서버/길드 규칙:
1) 서버보스는 이미 해당 server_id의 연합 전체 길드원이 후보로 제공됩니다. 특정 한 길드만 찾지 말고 후보 전체를 확인하세요.
2) 길드명/길드태그는 참가 대상 제한용이 아니라 닉네임 식별 보조 증거입니다.
3) 같은 닉네임 또는 매우 비슷한 닉네임이 여러 길드에 있으면 화면에 보이는 길드명/태그를 함께 확인해 후보키를 구분하세요.
4) 길드명이 가려져 동일 닉네임 후보를 구분할 수 없으면 확정 M으로 만들지 말고 Q로 남기세요.
...
4) 후보 명단과 명확히 일치하면 M 행으로 반환하세요.
5) 화면에는 보이지만 후보 명단에 없는 닉네임은 U 행으로 반환하세요.
6) 비슷하지만 확정할 수 없으면 Q 행으로 반환하세요.
7) 정중앙 플레이어 닉네임은 세 번 모두 반드시 재확인하세요.
8) 출력은 아래 한 줄 형식만 사용하고 설명문, 마크다운, JSON을 절대 쓰지 마세요.
...
Q|킹쥬쥬|c2`;
  ...
  const body={contents:[{role:"user",parts}],generationConfig:{mediaResolution:"MEDIA_RESOLUTION_HIGH",maxOutputTokens:2600,temperature:0.1}};
  ...
  let raw=geminiResultText_(data);
  if(!raw)throw new Error(`Gemini 응답 본문이 비어 있습니다. (${model||primary})`);
  raw=String(raw).replace(/```[a-z]*|```/gi,"").trim();

  const memberIds=new Set(),uncertain=[],notes=[],seenLines=new Set();
  const addUncertain=(t)=>{t=String(t||"").trim();if(t&&!seenLines.has(t)){seenLines.add(t);uncertain.push({text:t});}};

  /* V6.2.2 기본: 줄 단위 프로토콜. JSON 파싱에 의존하지 않는다. */
  let parsedAny=false;
  for(const line0 of raw.split(/\r?\n/)){
    const line=String(line0||"").trim();if(!line)continue;
    const p=line.split("|").map(x=>x.trim()),kind=String(p[0]||"").toUpperCase();
    if(kind==="M"&&p.length>=3){
      const c=keyMap.get(p[1]);if(c){memberIds.add(c.member_id);parsedAny=true;}
    }else if(kind==="U"&&p.length>=2){
      const seen=p.slice(1).join("|").trim();if(seen){
        const m=screenshotMatchDetectedName_(seen,candidates);
        if(m.id)memberIds.add(m.id);
        else if(m.hint)addUncertain(`${seen} → 후보 ${m.hint}`);
        else addUncertain(`미등록/대상외: ${seen}`);
        parsedAny=true;
      }
    }else if(kind==="Q"&&p.length>=2){
      const seen=String(p[1]||"").trim(),c=keyMap.get(String(p[2]||"").trim());
      addUncertain(c?`${seen||"판독불명"} → 후보 ${c.nickname}`:(seen||"판독불명"));
      parsedAny=true;
    }else if(kind==="N"&&p.length>=2){
      notes.push(p.slice(1).join("|").trim());parsedAny=true;
    }
  }

  /* 모델이 형식을 어기고 JSON으로 응답한 경우도 살려낸다. */
  if(!parsedAny&&raw.includes("{")){
    let obj=null,clean=raw.trim();
    try{obj=JSON.parse(clean)}catch(_){
      const a=clean.indexOf("{"),b=clean.lastIndexOf("}");
      if(a>=0&&b>a){try{obj=JSON.parse(clean.slice(a,b+1).replace(/,\s*([}\]])/g,"$1"))}catch(__){}}
    }
    if(obj){
      for(const m of Array.isArray(obj.matches)?obj.matches:[]){
        const key=String(m&&m.candidate_key||m&&m.key||"").trim(),c=keyMap.get(key);
        if(c){memberIds.add(c.member_id);parsedAny=true;}
        else{
          const id=String(m&&m.member_id||"").trim();
          if(id&&candidates.some(x=>String(x.member_id)===id)){memberIds.add(id);parsedAny=true;}
        }
      }
      for(const name of Array.isArray(obj.observed_unregistered)?obj.observed_unregistered:[]){
        const seen=String(name||"").trim();if(!seen)continue;
        const m=screenshotMatchDetectedName_(seen,candidates);
        if(m.id)memberIds.add(m.id);else if(m.hint)addUncertain(`${seen} → 후보 ${m.hint}`);else addUncertain(`미등록/대상외: ${seen}`);
        parsedAny=true;
      }
      for(const u of Array.isArray(obj.uncertain)?obj.uncertain:[]){
        const seen=String(u&&u.seen_text||"").trim(),key=String(u&&u.candidate_key||u&&u.key||"").trim(),c=keyMap.get(key);
        addUncertain(c?`${seen||"판독불명"} → 후보 ${c.nickname}`:(seen||"판독불명"));parsedAny=true;
      }
      if(obj.note)notes.push(String(obj.note).slice(0,300));
    }
  }

  if(!parsedAny)throw new Error(`Gemini 결과 해석 실패: ${raw.slice(0,260)}`);
  return {model:`Gemini Free Vision / ${model} HIGH · 3PASS`,member_ids:[...memberIds],uncertain,note:`one_call_3pass=1 | views=${views.length} candidates=${candidates.length}${notes.length?" | "+notes.join(" | ").slice(0,420):""}`};
}
