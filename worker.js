...
7) 작은 한글 글자를 우선해서 세밀하게 읽으세요.`;
  ...
  const res=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`,{method:"POST",headers:{"Content-Type":"application/json","x-goog-api-key":String(env.GEMINI_API_KEY)},body:JSON.stringify(body)});
  const text=await res.text();let data={};try{data=JSON.parse(text)}catch{}
  if(!res.ok){const msg=data&&data.error&&data.error.message?data.error.message:text.slice(0,300);throw new Error(`Gemini API ${res.status}: ${msg}`);}
  let raw=geminiResultText_(data);if(!raw)throw new Error("Gemini 응답 본문이 비어 있습니다.");
  raw=raw.replace(/^```(?:json)?\s*/i,"").replace(/\s*```$/i,"").trim();
