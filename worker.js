...
async function currentScreenshotCheck_(env,aid,row){
  ...
  const attendance=await botAttendanceList_(env,aid,pub.event_id),attendedIds=new Set((attendance.participants||[]).map(x=>String(x.member_id||"")).filter(Boolean));
  const recognizedIds=new Set(recognized.map(x=>String(x.member_id||"")));
  pub.matched=recognized.filter(x=>attendedIds.has(String(x.member_id||"")));
  pub.screenshot_only=recognized.filter(x=>!attendedIds.has(String(x.member_id||"")));
  pub.attendance_only=(attendance.participants||[]).filter(x=>!recognizedIds.has(String(x.member_id||""))).map(x=>({member_id:String(x.member_id||""),nickname:String(x.nickname||""),guild_id:String(x.guild_id||""),guild_name:String(x.guild_name||""),server_id:String(x.server_id||""),server_name:String(x.server_name||"")}));
...
async function analyzeScreenshotCore_(env,aid,eventId,images,meta={}){
  ...
  return currentScreenshotCheck_(env,aid,await first(env.DB,"SELECT * FROM attendance_screenshot_checks WHERE check_id=?",checkId));
}
