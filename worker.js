...
async function getAttendanceRoster(env,allianceId,eventId,guildId,authToken){
  ...
  return {event:{...e,display_time:formatKst(e.cut_at,false)},members:out,participant_count:out.filter(x=>x.attended).length,can_edit:perm.can_edit,scope:gid?"guild":"alliance",screenshot_check:await latestScreenshotCheckByEvent_(env,aid,e.event_id)};
}
...
async function screenshotCandidates_(env,aid,e){
  let rows;if(String(e.boss_scope||"WORLD")==="SERVER")rows=await all(env.DB,"SELECT m.member_id,m.nickname,m.guild_id,g.guild_name,g.server_id,s.server_name FROM members m JOIN guilds g ON g.guild_id=m.guild_id LEFT JOIN alliance_servers s ON s.alliance_id=g.alliance_id AND s.server_id=g.server_id WHERE g.alliance_id=? AND g.server_id=? AND g.status='active' AND m.status='활성' ORDER BY g.guild_name,m.nickname",aid,String(e.server_id||""));
  else rows=await all(env.DB,"SELECT m.member_id,m.nickname,m.guild_id,g.guild_name,g.server_id,s.server_name FROM members m JOIN guilds g ON g.guild_id=m.guild_id LEFT JOIN alliance_servers s ON s.alliance_id=g.alliance_id AND s.server_id=g.server_id WHERE g.alliance_id=? AND g.status='active' AND m.status='활성' ORDER BY s.server_name,g.guild_name,m.nickname",aid);
  return rows.map(x=>({member_id:String(x.member_id),nickname:String(x.nickname||""),guild_id:String(x.guild_id||""),guild_name:String(x.guild_name||""),server_id:String(x.server_id||""),server_name:String(x.server_name||"")}));
}
...
function bytesBase64_(bytes){
  const u=bytes instanceof Uint8Array?bytes:new Uint8Array(bytes||0);let out="",step=0x8000;
  for(let i=0;i<u.length;i+=step)out+=String.fromCharCode(...u.subarray(i,Math.min(i+step,u.length)));
  return btoa(out);
}
function screenshotImageStream_(bytes,mime){return new Blob([bytes],{type:String(mime||"image/jpeg")}).stream();}
async function screenshotTransform_(env,bytes,mime,region,label){
  ...
  const sourceW=Math.max(1,Number(info&&info.width||0)),fracW=Math.max(.1,Number(region.width||1));
  const cropW=Math.max(1,Math.round(sourceW*fracW)),targetW=Math.min(2304,Math.max(1280,Math.round(cropW*2)));
  async function build(upscale){
    const response=(await env.IMAGES.input(screenshotImageStream_(bytes,mime))
      ...
      .transform({width:targetW,fit:"scale-up",upscale,sharpen:2})
      .output({format:"image/jpeg",quality:94,anim:false})).response();
    if(!response.ok)throw new Error(`Images transform HTTP ${response.status}`);
    return new Uint8Array(await response.arrayBuffer());
  }
  let out,mode="generate";
  try{out=await build("generate");}catch(e){mode="interpolate";out=await build("interpolate");}
  return {label,data:bytesBase64_(out),mime_type:"image/jpeg",note:`${label} ${sourceW}w→${targetW}w ${mode}`};
}
async function screenshotVisionViews_(env,img){
  if(!env.IMAGES)throw new Error("Cloudflare Images 바인딩(IMAGES)이 설정되지 않았습니다. wrangler.jsonc의 images.binding=IMAGES를 확인하세요.");
  const bytes=base64Bytes_(img.data),mime=String(img.mime_type||"image/jpeg");
  /* 60% 크기의 겹치는 2x2 + 중앙 76%: 작은 캐릭터 닉네임을 크게 만든 뒤 Vision에 전달 */
  const regions=[
    {label:"center",top:.12,left:.12,width:.76,height:.76},
    {label:"top-left",top:0,left:0,width:.62,height:.62},
    {label:"top-right",top:0,left:.38,width:.62,height:.62},
    {label:"bottom-left",top:.38,left:0,width:.62,height:.62},
    {label:"bottom-right",top:.38,left:.38,width:.62,height:.62}
  ];
  const out=[],failed=[];
  for(const r of regions){
    try{out.push(await screenshotTransform_(env,bytes,mime,r,r.label));}
    catch(e){failed.push(`${r.label}: ${e&&e.message?e.message:String(e)}`);}
  }
  if(!out.length)return [{label:"original-fallback",data:String(img.data||""),mime_type:mime,note:`Images transform fallback: ${failed.join(" / ")}`}];
  if(failed.length)out[0].note+=` | failed ${failed.join(" / ")}`;
  return out;
}
async function workersAiAnalyzeView_(env,model,view,rosterJson,schema,candidates){
  const prompt=`이 이미지는 게임 보스 레이드 스크린샷의 확대 구역(${view.label})입니다. 이 구역 안에서 캐릭터 머리 위/주변에 표시된 플레이어 닉네임만 아주 세밀하게 읽으세요.\n등록 후보 목록에 실제로 존재하는 플레이어가 화면에서 확인될 때만 그 member_id를 matched_member_ids에 넣으세요. 작은 글자는 한글 자모/영문 모양을 후보 닉네임과 비교해 보정할 수 있지만 보이지 않는 사람을 추측하면 안 됩니다. 길드명, 보스명, 데미지 숫자, UI 메뉴, 스킬명, 채팅 문구는 닉네임으로 세지 마세요. 후보에 없거나 읽기 애매한 닉네임 비슷한 문자열은 uncertain_text에 넣으세요.\n출력 필드는 matched_member_ids, uncertain_text, note 세 개뿐입니다.\n\n[등록 후보]\n${rosterJson}`;
  const req={messages:[{role:"system",content:"작고 흐린 게임 플레이어 닉네임을 읽어 등록 명단과 대조하는 비전 분석기입니다."},{role:"user",content:prompt}],image:String(view.data||""),temperature:0,max_completion_tokens:1000,chat_template_kwargs:{enable_thinking:false}};
  let result,structured=true;
  try{result=await env.AI.run(model,{...req,response_format:{type:"json_schema",json_schema:schema}});}
  catch(e){
    const em=String(e&&e.message?e.message:e);
    if(!/json|schema|response[_ -]?format|unsupported|couldn.?t be met/i.test(em))throw new Error(`Workers AI 확대 이미지 분석 실패(${view.label}): ${em}`);
    structured=false;
    try{result=await env.AI.run(model,{...req,messages:[{role:"system",content:"작고 흐린 게임 플레이어 닉네임을 읽어 등록 명단과 대조하는 비전 분석기입니다. 반드시 JSON 객체 하나만 반환하세요."},{role:"user",content:prompt+'\n반드시 {"matched_member_ids":[],"uncertain_text":[],"note":""} 형태의 JSON 하나만 반환하세요.'}]});}
    catch(e2){throw new Error(`Workers AI 확대 이미지 분석 실패(${view.label}): ${e2&&e2.message?e2.message:String(e2)}`);}
  }
  const out=normalizeWorkersAiMatch_(workersAiJson_(result)),allowed=new Set(candidates.map(x=>x.member_id));
  return {ids:out.matched_member_ids.filter(id=>allowed.has(id)),uncertain:out.uncertain_text,note:out.note,structured};
}
async function workersAiScreenshotMatch_(env,images,candidates){
  if(!env.AI)throw new Error("Cloudflare Workers AI 바인딩(AI)이 설정되지 않았습니다. wrangler.jsonc에 ai.binding=AI를 추가하세요.");
  if(!env.IMAGES)throw new Error("Cloudflare Images 바인딩(IMAGES)이 설정되지 않았습니다. wrangler.jsonc에 images.binding=IMAGES를 추가하세요.");
  ...
  const roster=candidates.map(x=>({member_id:x.member_id,nickname:x.nickname,guild:x.guild_name,server:x.server_name}));
  const memberIds=new Set(),uncertain=new Set(),notes=[],rosterJson=JSON.stringify(roster);
  const schema={type:"object",additionalProperties:false,properties:{matched_member_ids:{type:"array",items:{type:"string"}},uncertain_text:{type:"array",items:{type:"string"}},note:{type:"string"}},required:["matched_member_ids","uncertain_text","note"]};
  for(let i=0;i<valid.length;i++){
    const views=await screenshotVisionViews_(env,valid[i]);
    notes.push(`image ${i+1}: ${views.map(v=>v.note).join(", ")}`);
    /* 한 장당 5개 확대 구역을 병렬 분석. 겹침 영역 중복은 Set으로 제거한다. */
    const results=await Promise.all(views.map(v=>workersAiAnalyzeView_(env,model,v,rosterJson,schema,candidates)));
    for(let j=0;j<results.length;j++){
      const out=results[j];for(const id of out.ids)memberIds.add(id);
      for(const t of out.uncertain){const v=String(t||"").trim();if(v)uncertain.add(v);}
      if(out.note)notes.push(`image ${i+1}/${views[j].label}: ${out.note}`);
      if(!out.structured)notes.push(`image ${i+1}/${views[j].label}: JSON schema fallback`);
    }
  }
  return {model:`Workers AI Vision + Images 5-crop upscale / ${model}`,member_ids:[...memberIds],uncertain:[...uncertain].map(text=>({text})),note:notes.join(" | ")};
}
async function analyzeScreenshotCore_(env,aid,eventId,images,meta={}){
  ...
  const candidates=await screenshotCandidates_(env,aid,e),analysis=await workersAiScreenshotMatch_(env,images,candidates),byId=new Map(candidates.map(x=>[x.member_id,x])),recognized=analysis.member_ids.map(id=>byId.get(id)).filter(Boolean);
...
const RPC = {
  ...
  saveAllianceNotice, saveGuildNotice,
  saveGuildMember, resetMemberWebPin, updateGuildMemberRole, updateMemberAllianceRole,
  saveGuildLoot, markGuildLootSold, deleteGuildLoot, addFundTransaction, getGuildStats,
  getAttendanceEvents, getAttendanceRoster, setAttendanceMember, analyzeAttendanceScreenshots, applyAttendanceScreenshotMissing,
  saveAllianceBoss, disableAllianceBoss, registerAllianceBossCut, setAllianceBossSpawn, closeBossAttendance
};
