...
async function geminiAnalyzeScreenshot_(env,img,candidates,context={}){
  ...
  const keyed=candidates.map((c,i)=>({key:`c${i+1}`,member_id:String(c.member_id),nickname:String(c.nickname||""),guild_name:String(c.guild_name||"")}));
  const keyMap=new Map(keyed.map(x=>[x.key,x]));
  const roster=keyed.map(c=>`${c.key}\t${c.nickname}\t${c.guild_name}`).join("\n");
...
Q|킹쥬쥬|c2`;
  ...
  return {model:`Gemini Free Vision / ${model} HIGH`,member_ids:[...memberIds],uncertain,note:`views=${views.length} candidates=${candidates.length}${notes.length?" | "+notes.join(" | ").slice(0,420):""}`};
}
...
✅ 스샷 확인+출석 ${(r.matched||[]).length}명 · ${names(r.matched)}
➕ 스샷 확인+미출석 ${(r.screenshot_only||[]).length}명 · ${names(r.screenshot_only)}
🔎 출석됨·회차 스샷 누적 미확인 ${(r.attendance_only||[]).length}명 · ${names(r.attendance_only)}
