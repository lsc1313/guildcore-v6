...
async function saveGuildLootScreenshotLocal(env,guildId,payload={},authToken){
  ...
  const prev=await first(env.DB,"SELECT * FROM loot_screenshot_imports WHERE guild_id=? AND image_hash=? AND source='browser_local_ocr' ORDER BY created_at DESC LIMIT 1",gid,hash);
  if(prev){let saved=[];try{saved=JSON.parse(prev.items_json||"[]")}catch{}return {ok:true,analysis_id:prev.analysis_id,items:saved,duplicate:true,imported_at:prev.imported_at||"",model:prev.model||"Browser Tesseract Local"};}
  const analysisId=uid("lootshot"),now=nowIso(),raw=String(payload.raw_text||"").slice(0,6000);
