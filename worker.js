...
async function bossView(env,allianceId){
  ...
  return bosses.map(b=>{
    ...
    const as=a.boss_scope==="SERVER"?0:1, bs=b.boss_scope==="SERVER"?0:1;
    if(as!==bs)return as-bs;
    if(as===0){
      const sn=String(a.server_name||"").localeCompare(String(b.server_name||""),"ko",{numeric:true});
      if(sn)return sn;
    }
    const an=String(a.next_spawn_at||"9999-12-31 23:59"), bn=String(b.next_spawn_at||"9999-12-31 23:59");
    const nt=an.localeCompare(bn);
    if(nt)return nt;
    return String(a.boss_name||"").localeCompare(String(b.boss_name||""),"ko");
  });
}
...
function normalizeLootRarity_(value){
  const r=String(value||"").trim();return ["희귀","영웅","전설"].includes(r)?r:"";
}
async function ensureLootScreenshotSchema_(env){
  await run(env.DB,"CREATE TABLE IF NOT EXISTS loot_screenshot_imports(analysis_id TEXT PRIMARY KEY,guild_id TEXT NOT NULL,image_hash TEXT NOT NULL,items_json TEXT NOT NULL DEFAULT '[]',raw_text TEXT NOT NULL DEFAULT '',model TEXT NOT NULL DEFAULT '',created_at TEXT NOT NULL,imported_at TEXT NOT NULL DEFAULT '',source TEXT NOT NULL DEFAULT 'cloudflare')");
  await run(env.DB,"CREATE UNIQUE INDEX IF NOT EXISTS idx_loot_screenshot_hash ON loot_screenshot_imports(guild_id,image_hash)");
  const cols=await all(env.DB,"PRAGMA table_info(loot)");
  if(!cols.some(x=>String(x.name||"")==="rarity"))await run(env.DB,"ALTER TABLE loot ADD COLUMN rarity TEXT NOT NULL DEFAULT ''");
}
async function sha256BytesHex_(bytes){
  const buf=await crypto.subtle.digest("SHA-256",bytes instanceof Uint8Array?bytes:new Uint8Array(bytes||0));
  return [...new Uint8Array(buf)].map(x=>x.toString(16).padStart(2,"0")).join("");
}
function lootVisionAnswer_(r){
...
function parseLootVision_(raw){
  const out=[];
  const text=String(raw||"").replace(/```[^\n]*\n?/g,"").replace(/```/g,"").trim();
  for(const line0 of text.split(/\r?\n/)){
    let line=String(line0||"").trim().replace(/^[-*•]\s*/,"");if(!line)continue;
    const p=line.split("|").map(x=>x.trim());
    if(String(p[0]||"").toUpperCase()!=="I"||p.length<4)continue;
    const rarity=normalizeLootRarity_(p[1]);let bound=/^(1|true|yes|y|귀속)$/i.test(String(p[2]||""));
    let name=p.slice(3).join("|").trim().replace(/^['\"]|['\"]$/g,"").trim();
    name=name.replace(/\s*[을를]\s*획득하였습니다\.?\s*$/,"" ).trim();
    if(!name)continue;if(name.includes("귀속"))bound=true;
    out.push({index:out.length,item_name:name,rarity,bound,registerable:Boolean(rarity&&!bound),confidence:""});
  }
  return out;
}
async function analyzeGuildLootScreenshot(env,guildId,img={},authToken){
  await requireGuildOperate(env,authToken,guildId);
  throw new Error("아이템 스크린샷 OCR은 V6.2.7부터 휴대폰 브라우저 로컬 처리입니다. 화면을 새로고침한 뒤 다시 분석하세요.");
}
async function saveGuildLootScreenshotLocal(env,guildId,payload={},authToken){
  await requireGuildOperate(env,authToken,guildId);await ensureLootScreenshotSchema_(env);
  const gid=String(guildId),hash=String(payload.image_hash||"").trim().toLowerCase();
  if(!/^[a-f0-9]{64}$/.test(hash))throw new Error("스크린샷 해시가 올바르지 않습니다.");
  const rawItems=Array.isArray(payload.items)?payload.items:[];if(rawItems.length>60)throw new Error("한 번에 처리할 아이템이 너무 많습니다.");
  const items=[];
  for(const x of rawItems){let name=String(x&&x.item_name||"").trim().slice(0,180),rarity=normalizeLootRarity_(x&&x.rarity||""),bound=Boolean(x&&x.bound)||name.includes("귀속");if(!name||!rarity)continue;items.push({index:items.length,item_name:name,rarity,bound,registerable:!bound,confidence:String(x&&x.confidence||"").slice(0,30)});}
  if(!items.length)throw new Error("저장할 로컬 OCR 결과가 없습니다.");
  /* 같은 이미지 해시는 엔진(source)과 무관하게 1건만 존재한다.
     V6.2.6 Cloudflare OCR로 이미 분석했던 스샷을 V6.2.7+ 로컬 OCR로 다시 테스트할 때
     기존 row를 못 찾고 INSERT하여 UNIQUE(guild_id,image_hash) 충돌하던 문제를 방지한다. */
  const prev=await first(env.DB,"SELECT * FROM loot_screenshot_imports WHERE guild_id=? AND image_hash=? ORDER BY created_at DESC LIMIT 1",gid,hash);
  const now=nowIso(),raw=String(payload.raw_text||"").slice(0,6000);
  if(prev){
    let saved=[];try{saved=JSON.parse(prev.items_json||"[]")}catch{}
    if(prev.imported_at){
      return {ok:true,analysis_id:prev.analysis_id,items:saved,duplicate:true,imported_at:prev.imported_at||"",model:prev.model||"이전 OCR",message:"이미 아이템 내역에 반영한 스크린샷입니다."};
    }
    /* 아직 등록하지 않은 과거 분석이면 같은 analysis_id를 유지한 채 최신 로컬 OCR 결과로 교체한다. */
    await run(env.DB,"UPDATE loot_screenshot_imports SET items_json=?,raw_text=?,model=?,created_at=?,source='browser_local_ocr' WHERE guild_id=? AND image_hash=?",JSON.stringify(items),raw,"Browser Tesseract.js 5 Local",now,gid,hash);
    return {ok:true,analysis_id:prev.analysis_id,items,duplicate:false,reanalyzed:true,imported_at:"",model:"Browser Tesseract.js 5 Local",message:"기존 스크린샷 분석을 로컬 OCR 결과로 갱신했습니다."};
  }
  const analysisId=uid("lootshot");
  try{
    await run(env.DB,"INSERT INTO loot_screenshot_imports(analysis_id,guild_id,image_hash,items_json,raw_text,model,created_at,imported_at,source) VALUES(?,?,?,?,?,?,?,'','browser_local_ocr')",analysisId,gid,hash,JSON.stringify(items),raw,"Browser Tesseract.js 5 Local",now);
  }catch(e){
    /* 동시에 같은 스샷을 두 번 분석한 경우에도 UNIQUE 에러를 사용자에게 노출하지 않는다. */
    if(/UNIQUE constraint failed|SQLITE_CONSTRAINT_UNIQUE/i.test(String(e&&e.message||e))){
      const again=await first(env.DB,"SELECT * FROM loot_screenshot_imports WHERE guild_id=? AND image_hash=? LIMIT 1",gid,hash);
      if(again){let saved=[];try{saved=JSON.parse(again.items_json||"[]")}catch{}return {ok:true,analysis_id:again.analysis_id,items:saved,duplicate:true,imported_at:again.imported_at||"",model:again.model||"Browser Tesseract Local"};}
    }
    throw e;
  }
  return {ok:true,analysis_id:analysisId,items,duplicate:false,imported_at:"",model:"Browser Tesseract.js 5 Local"};
}
async function importGuildLootScreenshot(env,guildId,analysisId,selections=[],authToken){
  await requireGuildOperate(env,authToken,guildId);await ensureLootScreenshotSchema_(env);
  const gid=String(guildId),row=await first(env.DB,"SELECT * FROM loot_screenshot_imports WHERE guild_id=? AND analysis_id=?",gid,String(analysisId||""));
  if(!row)throw new Error("스크린샷 분석 결과를 찾을 수 없습니다.");if(row.imported_at)throw new Error("이미 아이템 내역에 반영한 스크린샷입니다.");
  let items=[];try{items=JSON.parse(row.items_json||"[]")}catch{};
  const chosen=new Map();for(const v of (Array.isArray(selections)?selections:[])){if(typeof v==="number"){chosen.set(Number(v),{})}else if(v&&Number.isFinite(Number(v.index))){chosen.set(Number(v.index),{item_name:String(v.item_name||"").trim().slice(0,180),rarity:normalizeLootRarity_(v.rarity||"")})}}
  const groups=new Map();for(const x of items){const ix=Number(x.index),ov=chosen.get(ix);if(!ov)continue;const name=String(ov.item_name||x.item_name||"").trim(),rarity=normalizeLootRarity_(ov.rarity||x.rarity||""),bound=Boolean(x.bound)||name.includes("귀속");if(!name||!rarity||bound)continue;const key=rarity+"\u0000"+name,g=groups.get(key)||{item_name:name,rarity,quantity:0};g.quantity++;groups.set(key,g);}
  if(!groups.size)throw new Error("등록할 수 있는 아이템이 선택되지 않았습니다.");
  const now=nowIso(),lootSource=String(row.source||"")==="browser_local_ocr"?"screenshot_local_ocr":"screenshot_cloudflare";
  for(const g of groups.values())await run(env.DB,"INSERT INTO loot(loot_id,guild_id,raid_id,item_name,quantity,status,winner_member_id,sale_amount,fund_rate,fund_amount,guild_support_amount,distribution_pool,completed_at,note,created_at,source,rarity) VALUES(?,?,?,?,?,'보유','',NULL,NULL,NULL,NULL,NULL,'','',?,?,?)",uid("loot"),gid,"",g.item_name,g.quantity,now,lootSource,g.rarity);
  await run(env.DB,"UPDATE loot_screenshot_imports SET imported_at=? WHERE analysis_id=? AND guild_id=?",now,String(analysisId),gid);
  const total=[...groups.values()].reduce((n,x)=>n+x.quantity,0);return {ok:true,count:total,message:`스크린샷 아이템 ${total}건을 등록했습니다.`};
}
async function saveGuildLoot(env,guildId,payload={},authToken){await requireGuildOperate(env,authToken,guildId);await ensureLootScreenshotSchema_(env);const item=String(payload.item_name||"").trim(),rarity=normalizeLootRarity_(payload.rarity||""),qty=Math.max(1,Number(payload.quantity||1)),status=String(payload.status||"보유"),sale=Math.max(0,Number(payload.sale_amount||0));if(!item)throw new Error("아이템명을 입력하세요.");if(!["보유","판매완료","지급완료"].includes(status))throw new Error("아이템 상태가 올바르지 않습니다.");await run(env.DB,"INSERT INTO loot(loot_id,guild_id,raid_id,item_name,quantity,status,winner_member_id,sale_amount,fund_rate,fund_amount,guild_support_amount,distribution_pool,completed_at,note,created_at,source,rarity) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",uid("loot"),String(guildId),String(payload.raid_id||""),item,qty,status,String(payload.winner_member_id||""),sale||null,null,null,null,null,(status==="판매완료"||status==="지급완료")?nowIso():"",String(payload.note||""),nowIso(),String(payload.source||"web"),rarity);return {ok:true,message:"아이템을 등록했습니다."};}
...
async function ensureAttendanceEvidenceSchema_(env){
  await run(env.DB,"CREATE TABLE IF NOT EXISTS attendance_source_evidence(evidence_id TEXT PRIMARY KEY,alliance_id TEXT NOT NULL,event_id TEXT NOT NULL,member_id TEXT NOT NULL,source_key TEXT NOT NULL,first_seen_at TEXT NOT NULL,last_seen_at TEXT NOT NULL,note TEXT NOT NULL DEFAULT '')");
  await run(env.DB,"CREATE UNIQUE INDEX IF NOT EXISTS idx_att_evidence_unique ON attendance_source_evidence(alliance_id,event_id,member_id,source_key)");
  await run(env.DB,"CREATE INDEX IF NOT EXISTS idx_att_evidence_event ON attendance_source_evidence(alliance_id,event_id,member_id)");
}
function attendanceCanonicalSource_(source){
  const s=String(source||"").toLowerCase();
  if(s.includes("screenshot"))return "screenshot";
  if(s.includes("kakao"))return "kakao";
  if(s.includes("discord"))return "discord";
  if(s.includes("web")||s.includes("admin"))return "web";
  return "";
}
function attendanceSourceLabelKey_(key){
  const k=String(key||"").toLowerCase();
  if(k==="kakao")return "Kakao";
  if(k==="discord")return "Discord";
  if(k==="screenshot")return "스크린샷";
  if(k==="web")return "웹";
  return k;
}
const ATTENDANCE_SOURCE_ORDER_={kakao:1,discord:2,web:3,screenshot:4};
async function recordAttendanceEvidence_(env,aid,eventId,memberId,source,note=""){
  const key=attendanceCanonicalSource_(source),eid=String(eventId||""),mid=String(memberId||"");if(!key||!eid||!mid)return;
  await ensureAttendanceEvidenceSchema_(env);const now=nowIso();
  await run(env.DB,"INSERT INTO attendance_source_evidence(evidence_id,alliance_id,event_id,member_id,source_key,first_seen_at,last_seen_at,note) VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(alliance_id,event_id,member_id,source_key) DO UPDATE SET last_seen_at=excluded.last_seen_at,note=CASE WHEN excluded.note<>'' THEN excluded.note ELSE attendance_source_evidence.note END",uid("evd"),String(aid||""),eid,mid,key,now,now,String(note||""));
}
async function removeAttendanceEvidence_(env,aid,eventId,memberId,source){
  const key=attendanceCanonicalSource_(source),eid=String(eventId||""),mid=String(memberId||"");if(!key||!eid||!mid)return;
  await ensureAttendanceEvidenceSchema_(env);
  await run(env.DB,"DELETE FROM attendance_source_evidence WHERE alliance_id=? AND event_id=? AND member_id=? AND source_key=?",String(aid||""),eid,mid,key);
}
async function attendanceExplicitSourcesForMember_(env,aid,eventId,memberId){
  const eid=String(eventId||""),mid=String(memberId||""),keys=new Set();if(!eid||!mid)return [];
  await ensureAttendanceEvidenceSchema_(env);
  const rows=await all(env.DB,"SELECT source_key FROM attendance_source_evidence WHERE alliance_id=? AND event_id=? AND member_id=?",String(aid||""),eid,mid);
  for(const x of rows){const k=attendanceCanonicalSource_(x.source_key)||String(x.source_key||"").toLowerCase();if(k)keys.add(k);}
  /* 스크린샷 확인은 실제 현장 근거이므로 해당 입력 경로 취소와 별개로 유지한다. */
  await ensureScreenshotSchema_(env);
  const shots=await all(env.DB,"SELECT recognized_json FROM attendance_screenshot_checks WHERE alliance_id=? AND event_id=? AND status='done'",String(aid||""),eid);
  for(const row of shots)for(const item of jsonArray_(row.recognized_json)){if(String(item&&item.member_id||"")===mid)keys.add("screenshot");}
  return [...keys].sort((a,b)=>(ATTENDANCE_SOURCE_ORDER_[a]||99)-(ATTENDANCE_SOURCE_ORDER_[b]||99)||a.localeCompare(b));
}
async function attendanceEvidenceMap_(env,aid,eventId){
  await ensureAttendanceEvidenceSchema_(env);const eid=String(eventId||""),map=new Map(),add=(mid,key)=>{mid=String(mid||"");key=attendanceCanonicalSource_(key)||String(key||"").toLowerCase();if(!mid||!key)return;let set=map.get(mid);if(!set){set=new Set();map.set(mid,set)}set.add(key)};
  const rows=await all(env.DB,"SELECT member_id,source_key FROM attendance_source_evidence WHERE alliance_id=? AND event_id=?",String(aid||""),eid);for(const x of rows)add(x.member_id,x.source_key);
  /* 기존 단일 source 컬럼도 자동 승계. 단, 새 evidence가 없는 활성 참여만 보완한다. */
  const legacy=await all(env.DB,"SELECT member_id_at_event,source,attended FROM boss_attendance WHERE alliance_id=? AND event_id=?",String(aid||""),eid);
  for(const x of legacy){const mid=String(x.member_id_at_event||"");if(!mid||!Number(x.attended))continue;const set=map.get(mid);if(!set||!set.size)add(mid,x.source);}
  /* 과거/현재 스크린샷 대조 기록도 근거로 합산 */
  await ensureScreenshotSchema_(env);const shots=await all(env.DB,"SELECT recognized_json FROM attendance_screenshot_checks WHERE alliance_id=? AND event_id=? AND status='done'",String(aid||""),eid);
  for(const row of shots)for(const item of jsonArray_(row.recognized_json)){const mid=String(item&&item.member_id||"");if(mid)add(mid,"screenshot");}
  return map;
}
function attendanceSourcesFor_(map,memberId,legacySource=""){
  const keys=new Set(map.get(String(memberId||""))||[]),legacy=attendanceCanonicalSource_(legacySource);if(legacy)keys.add(legacy);
  const arr=[...keys].sort((a,b)=>(ATTENDANCE_SOURCE_ORDER_[a]||99)-(ATTENDANCE_SOURCE_ORDER_[b]||99)||a.localeCompare(b));
  return {sources:arr,source_labels:arr.map(attendanceSourceLabelKey_),source_label:arr.map(attendanceSourceLabelKey_).join(" · ")};
}
async function ensureAttendanceResultQueueSchema_(env){
  await run(env.DB,"CREATE TABLE IF NOT EXISTS attendance_result_queue(result_id TEXT PRIMARY KEY,alliance_id TEXT NOT NULL,event_id TEXT NOT NULL DEFAULT '',boss_id TEXT NOT NULL DEFAULT '',boss_name TEXT NOT NULL DEFAULT '',boss_scope TEXT NOT NULL DEFAULT 'WORLD',server_id TEXT NOT NULL DEFAULT '',server_name TEXT NOT NULL DEFAULT '',message TEXT NOT NULL DEFAULT '',discord_status TEXT NOT NULL DEFAULT 'queued',kakao_status TEXT NOT NULL DEFAULT 'queued',created_at TEXT NOT NULL,discord_sent_at TEXT NOT NULL DEFAULT '',kakao_sent_at TEXT NOT NULL DEFAULT '',discord_message_id TEXT NOT NULL DEFAULT '',discord_channel_id TEXT NOT NULL DEFAULT '',note TEXT NOT NULL DEFAULT '')");
}
function attendanceParticipantLine_(p){const labels=Array.isArray(p.source_labels)?p.source_labels:[];return `${p.nickname||""}${labels.length?` [${labels.join("+")}]`:""}`;}
async function buildAttendanceLiveMessage_(env,aid,eventId,check=null,reason=""){
  const data=await botAttendanceList_(env,aid,eventId),groups={};
  for(const p of data.participants||[]){const k=(p.server_name?`${p.server_name} / `:"")+(p.guild_name||"기타");(groups[k]||(groups[k]=[])).push(p);}
  const lines=[reason?`📸 ${data.boss_name} ${reason}`:`📋 ${data.boss_name} 참여 명단 최신화`,`현재 참여 ${(data.participants||[]).length}명`];
  if(check)lines.push(`스샷 누적 확인 ${Number(check.recognized_count||0)}명 · 분석 ${Number(check.check_count||1)}회`);
  for(const k of Object.keys(groups).sort((a,b)=>a.localeCompare(b,"ko"))){lines.push(`[${k}]`);lines.push(groups[k].map(attendanceParticipantLine_).join(" · "));}
  if(check&&Array.isArray(check.screenshot_only)&&check.screenshot_only.length)lines.push(`➕ 스샷 확인·미출석 ${check.screenshot_only.length}명: ${check.screenshot_only.map(x=>x.nickname||x.member_id||"").filter(Boolean).join(" · ")}`);
  if(check&&Array.isArray(check.attendance_only)&&check.attendance_only.length)lines.push(`🔎 출석됨·스샷 누적 미확인 ${check.attendance_only.length}명: ${check.attendance_only.map(x=>x.nickname||x.member_id||"").filter(Boolean).join(" · ")}`);
  return {data,message:lines.join("\n").slice(0,3800)};
}
async function queueKakaoAttendanceLive_(env,aid,eventId,check=null,reason="스크린샷 대조"){
  await ensureAttendanceResultQueueSchema_(env);const e=await first(env.DB,"SELECT * FROM boss_events WHERE alliance_id=? AND event_id=?",String(aid||""),String(eventId||""));if(!e)return;
  const built=await buildAttendanceLiveMessage_(env,aid,eventId,check,reason),srv=e.server_id?await serverMeta(env,aid,e.server_id):null,now=nowIso();
  await run(env.DB,"INSERT INTO attendance_result_queue(result_id,alliance_id,event_id,boss_id,boss_name,boss_scope,server_id,server_name,message,discord_status,kakao_status,created_at,discord_message_id,discord_channel_id,note) VALUES(?,?,?,?,?,?,?,?,?,'sent','queued',?,'','','screenshot live update')",uid("result"),String(aid||""),e.event_id,e.boss_id,e.boss_name,e.boss_scope||"WORLD",e.server_id||"",srv?.server_name||"",built.message,now);
}
...
async function getAttendanceRoster(env,allianceId,eventId,guildId,authToken){
  ...
  let members;
  if(gid)members=await all(env.DB,"SELECT m.*,g.guild_name,g.server_id,g.game_server_name FROM members m JOIN guilds g ON g.guild_id=m.guild_id WHERE m.guild_id=? AND m.status='활성' ORDER BY m.nickname",gid);
  else if(String(e.boss_scope||"WORLD").toUpperCase()==="SERVER")members=await all(env.DB,"SELECT m.*,g.guild_name,g.server_id,g.game_server_name FROM members m JOIN guilds g ON g.guild_id=m.guild_id WHERE g.alliance_id=? AND g.server_id=? AND g.status='active' AND m.status='활성' ORDER BY g.created_at,g.guild_name,m.nickname",aid,String(e.server_id||""));
  else members=await all(env.DB,"SELECT m.*,g.guild_name,g.server_id,g.game_server_name FROM members m JOIN guilds g ON g.guild_id=m.guild_id WHERE g.alliance_id=? AND g.status='active' AND m.status='활성' ORDER BY g.created_at,g.guild_name,m.nickname",aid);
  const rows=await all(env.DB,"SELECT * FROM boss_attendance WHERE event_id=?",String(eventId)),byMember=new Map(rows.filter(x=>x.member_id_at_event).map(x=>[String(x.member_id_at_event),x])),evidence=await attendanceEvidenceMap_(env,aid,e.event_id);
  const out=members.map(m=>{const a=byMember.get(String(m.member_id));if(a)byMember.delete(String(m.member_id));const src=attendanceSourcesFor_(evidence,m.member_id,a?.source||"");return {member_id:m.member_id,guild_id:m.guild_id,guild_name:m.guild_name,nickname:m.nickname,role:normalizeGuildRole(m.role),attended:Boolean(a&&Number(a.attended)),source:a?.source||"",sources:src.sources,source_labels:src.source_labels,source_label:src.source_label,recorded_at:a?.recorded_at||""};});
  for(const a of byMember.values()){if(gid&&String(a.guild_id_at_event)!==gid)continue;if(!Number(a.attended))continue;if(!gid&&String(e.boss_scope||"WORLD").toUpperCase()==="SERVER"&&String(a.server_id_at_event||"")!==String(e.server_id||""))continue;const src=attendanceSourcesFor_(evidence,a.member_id_at_event,a.source||"");out.push({member_id:a.member_id_at_event||`past_${a.attendance_id}`,guild_id:a.guild_id_at_event||"",guild_name:a.guild_name_at_event||"과거 소속",nickname:a.game_nickname_at_event||"과거 길드원",role:"과거기록",attended:true,source:a.source||"",sources:src.sources,source_labels:src.source_labels,source_label:src.source_label,recorded_at:a.recorded_at||"",historical:true});}
...
function attendanceSourceLabel(source){const k=attendanceCanonicalSource_(source);return k?attendanceSourceLabelKey_(k):String(source||"");}
async function setAttendanceMember(env,allianceId,eventId,memberId,attended,source,authToken){
  ...
  const old=await first(env.DB,"SELECT * FROM boss_attendance WHERE event_id=? AND member_id_at_event=? ORDER BY recorded_at DESC LIMIT 1",String(eventId),String(memberId)),now=nowIso(),on=boolVal(attended,false),src=String(source||"web_admin");
  if(old&&old.source)await recordAttendanceEvidence_(env,aid,e.event_id,m.member_id,old.source,"legacy before web update");
  if(on){
    if(old){await run(env.DB,"UPDATE boss_attendance SET attended=1,recorded_at=?,cancelled_at='',source=?,guild_id_at_event=?,guild_name_at_event=?,server_id_at_event=?,server_name_at_event=?,game_nickname_at_event=? WHERE attendance_id=?",now,src,m.guild_id,m.guild_name,m.server_id||"",m.game_server_name||"",m.nickname,old.attendance_id);}else{const dm=await first(env.DB,"SELECT discord_user_id FROM discord_members WHERE alliance_id=? AND current_member_id=? AND status='active'",aid,String(memberId));await run(env.DB,"INSERT INTO boss_attendance(attendance_id,alliance_id,event_id,boss_id,discord_user_id,guild_id_at_event,guild_name_at_event,server_id_at_event,server_name_at_event,member_id_at_event,game_nickname_at_event,attended,recorded_at,cancelled_at,online_status_snapshot,source,note) VALUES(?,?,?,?,?,?,?,?,?,?,?,1,?,'','',?,'')",uid("att"),aid,e.event_id,e.boss_id,dm?.discord_user_id||"",m.guild_id,m.guild_name,m.server_id||"",m.game_server_name||"",m.member_id,m.nickname,now,src);}
    await recordAttendanceEvidence_(env,aid,e.event_id,m.member_id,src,"web attendance");
    await queueAttendanceSync_(env,aid,e.event_id,"upsert","web attendance change");
    return {ok:true,attended:true,requested_attended:true,message:`${m.nickname} 참여 추가`};
  }
  /* 체크 해제는 웹 경로만 취소한다. Discord/Kakao/스크린샷 근거가 남으면 실제 참여는 유지한다. */
  await removeAttendanceEvidence_(env,aid,e.event_id,m.member_id,src);
  const remaining=await attendanceExplicitSourcesForMember_(env,aid,e.event_id,m.member_id),keep=remaining.length>0,primary=keep?remaining[0]:src;
  if(old)await run(env.DB,"UPDATE boss_attendance SET attended=?,cancelled_at=?,source=?,guild_id_at_event=?,guild_name_at_event=?,server_id_at_event=?,server_name_at_event=?,game_nickname_at_event=? WHERE attendance_id=?",keep?1:0,keep?"":now,primary,m.guild_id,m.guild_name,m.server_id||"",m.game_server_name||"",m.nickname,old.attendance_id);
  await queueAttendanceSync_(env,aid,e.event_id,"upsert","web attendance source removed");
  const labels=remaining.map(attendanceSourceLabelKey_);
  return {ok:true,attended:keep,requested_attended:false,remaining_sources:remaining,remaining_source_labels:labels,message:keep?`${m.nickname} 웹 체크 해제 · 다른 확인 경로(${labels.join("+")})로 참여 유지`:`${m.nickname} 참여 제거`};
}
...
function publicScreenshotCheck_(x){
  if(!x)return null;
  const note=String(x.note||""),calls=note.match(/(?:^|\s|\|)api_calls=(\d+)/),passes=note.match(/(?:^|\s|\|)external_passes=(\d+)/);
  return {check_id:x.check_id,event_id:x.event_id,boss_id:x.boss_id,boss_name:x.boss_name,source:x.source,source_message_id:x.source_message_id||"",source_channel_id:x.source_channel_id||"",result_message_id:x.result_message_id||"",image_count:Number(x.image_count||0),recognized_count:Number(x.recognized_count||0),recognized:jsonArray_(x.recognized_json),matched:jsonArray_(x.matched_json),screenshot_only:jsonArray_(x.screenshot_only_json),attendance_only:jsonArray_(x.attendance_only_json),uncertain:jsonArray_(x.uncertain_json),model:x.model||"",analysis_api_calls:calls?Number(calls[1]):1,analysis_passes:passes?Number(passes[1]):1,status:x.status||"done",created_at:x.created_at||"",created_at_display:x.created_at?formatKst(x.created_at):"",note};
}
async function currentScreenshotCheck_(env,aid,row){
  const pub=publicScreenshotCheck_(row);if(!pub)return null;
  /* 과거 버전에서 각 스샷별로 따로 저장된 결과도 조회 시점에 전부 합쳐 회차 누적으로 복구한다. */
  const evidenceRows=await all(env.DB,"SELECT recognized_json,image_count FROM attendance_screenshot_checks WHERE alliance_id=? AND event_id=? AND status='done' ORDER BY created_at ASC",aid,String(pub.event_id||"")),merged=new Map();
  for(const r of evidenceRows)for(const item of jsonArray_(r.recognized_json)){const id=String(item&&item.member_id||"");if(id)merged.set(id,item);}
  const recognized=[...merged.values()];pub.recognized=recognized;
  const ev=await first(env.DB,"SELECT boss_scope,server_id FROM boss_events WHERE alliance_id=? AND event_id=?",aid,String(pub.event_id||""));
  pub.boss_scope=String(ev?.boss_scope||"WORLD").toUpperCase();pub.server_id=String(ev?.server_id||"");pub.server_name="";
  if(pub.server_id){const sm=await serverMeta(env,aid,pub.server_id);pub.server_name=String(sm?.server_name||"");}
  const attendance=await botAttendanceList_(env,aid,pub.event_id),attMap=new Map((attendance.participants||[]).map(x=>[String(x.member_id||""),x])),attendedIds=new Set(attMap.keys());
  const recognizedIds=new Set(recognized.map(x=>String(x.member_id||"")));
  pub.matched=recognized.filter(x=>attendedIds.has(String(x.member_id||""))).map(x=>{const a=attMap.get(String(x.member_id||""))||{};return {...x,sources:a.sources||[],source_labels:a.source_labels||[]};});
  pub.screenshot_only=recognized.filter(x=>!attendedIds.has(String(x.member_id||"")));
  pub.attendance_only=(attendance.participants||[]).filter(x=>!recognizedIds.has(String(x.member_id||""))).map(x=>({member_id:String(x.member_id||""),nickname:String(x.nickname||""),guild_id:String(x.guild_id||""),guild_name:String(x.guild_name||""),server_id:String(x.server_id||""),server_name:String(x.server_name||""),sources:x.sources||[],source_labels:x.source_labels||[]}));
  pub.recognized_count=recognized.length;pub.check_count=evidenceRows.length;pub.total_image_count=evidenceRows.reduce((n,x)=>n+Number(x.image_count||0),0);
  return pub;
}
async function latestScreenshotCheckByEvent_(env,aid,eventId){await ensureScreenshotSchema_(env);const row=await first(env.DB,"SELECT * FROM attendance_screenshot_checks WHERE alliance_id=? AND event_id=? AND status='done' ORDER BY created_at DESC LIMIT 1",aid,String(eventId||""));return currentScreenshotCheck_(env,aid,row);}
...
async function geminiScreenshotViews_(env,img,mode="full",multi=false){
  if(!env.IMAGES)throw new Error("Cloudflare Images 바인딩(IMAGES)이 설정되지 않았습니다. wrangler.jsonc의 images.binding=IMAGES를 확인하세요.");
  const bytes=base64Bytes_(img.data),mime=String(img.mime_type||"image/jpeg");
  const info=await env.IMAGES.info(screenshotImageStream_(bytes,mime));
  const w=Math.max(1,Number(info&&info.width||0)),h=Math.max(1,Number(info&&info.height||0)),landscape=w>=h;
  let regions=[],includeOriginal=true;
  if(mode==="pass1"&&multi){
    regions=landscape?[
    ...
    regions=landscape?[
      ...
      {label:"upper-center",top:0,left:.18,width:.64,height:.52}
    ]:[
      {label:"bottom",top:.48,left:.08,width:.84,height:.52},
      {label:"left-center",top:.16,left:0,width:.58,height:.68},
      {label:"right-center",top:.16,left:.42,width:.58,height:.68}
    ];
  }else{
    regions=landscape?[
      {label:"center",top:.14,left:.22,width:.56,height:.66},
  ...
  const views=[];
  if(includeOriginal)views.push({label:"original",data:String(img.data||""),mime_type:mime,note:`original ${w}x${h}`});
  const target=multi?1280:1536;
  for(const r of regions){
    try{views.push(await screenshotTransform_(env,bytes,mime,r,r.label,w,h,target));}
    catch(e){/* 한 확대뷰 실패 때문에 전체 묶음 분석을 버리지 않는다. */}
  }
  return views.filter(v=>v&&v.data);
}
async function geminiScreenshotBundleViews_(env,images,pass=1){
  const valid=(Array.isArray(images)?images:[]).slice(0,4).filter(x=>x&&x.data&&String(x.mime_type||"").startsWith("image/"));
  if(!valid.length)throw new Error("분석할 이미지가 없습니다.");
  const multi=valid.length>1,out=[];
  /* V6.2.18
     여러 장을 선택한 경우에는 '원본 N장 = Gemini 이미지 N개'로 그대로 보낸다.
     V6.2.17은 각 원본마다 확대뷰를 2~3개 더 만들어 3장이 9개 이미지 파트로
     부풀어 524/비JSON 응답 위험이 커졌다.
     2차 검증도 별도 API 호출에서 같은 원본 묶음을 다시 독립 판독한다. */
  if(multi){
    for(let i=0;i<valid.length;i++){
      const img=valid[i];
      out.push({
        label:`img${i+1}-original`,
        data:String(img.data||""),
        mime_type:String(img.mime_type||"image/jpeg"),
        note:`source image ${i+1}`
      });
    }
    return {valid,views:out};
  }
  /* 한 장 분석은 기존 고정밀 확대뷰 방식을 유지한다. */
  for(let i=0;i<valid.length;i++){
    const mode=pass===2?"pass2":"full",views=await geminiScreenshotViews_(env,valid[i],mode,false);
    for(const v of views)out.push({...v,label:`img${i+1}-${v.label}`});
  }
  return {valid,views:out};
}
function geminiScreenshotRoster_(candidates){
  const keyed=candidates.map((c,i)=>({key:`c${i+1}`,member_id:String(c.member_id),nickname:String(c.nickname||""),server_name:String(c.server_name||""),guild_name:String(c.guild_name||"")}));
  return {keyed,keyMap:new Map(keyed.map(x=>[x.key,x])),roster:keyed.map(c=>`${c.key}\t${c.nickname}\t${c.server_name}\t${c.guild_name}`).join("\n")};
}
...
async function geminiScreenshotApiCall_(env,parts,pass,deadline){
  const body={contents:[{role:"user",parts}],generationConfig:{mediaResolution:"MEDIA_RESOLUTION_HIGH",maxOutputTokens:3000,temperature:0}};
  ...
  const base=pass===1?[26000,11000,7000]:[18000,8000,6000];
  let data=null,model="",lastErr="",apiCalls=0;
  for(let mi=0;mi<models.length;mi++){
    const remain=Number(deadline||0)-Date.now();if(remain<5000)break;
    const m=models[mi],ms=Math.max(4000,Math.min(base[Math.min(mi,base.length-1)],remain-1500)),controller=new AbortController(),timer=setTimeout(()=>controller.abort(),ms);apiCalls++;
    try{
      const res=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(m)}:generateContent`,{method:"POST",headers:{"Content-Type":"application/json","x-goog-api-key":String(env.GEMINI_API_KEY)},body:JSON.stringify(body),signal:controller.signal});
      const text=await res.text();let parsed={};try{parsed=JSON.parse(text)}catch{}
      if(res.ok){data=parsed;model=m;break;}
      const msg=parsed&&parsed.error&&parsed.error.message?parsed.error.message:text.slice(0,300);lastErr=`Gemini API ${res.status} (${m}): ${msg}`;
      if(![408,429,500,502,503,504].includes(res.status))throw new Error(lastErr);
    }catch(err){
      if(err&&err.name==="AbortError")lastErr=`Gemini 응답 시간초과 (${m}, ${Math.round(ms/1000)}초)`;
      else if(String(err&&err.message||"").startsWith("Gemini API "))throw err;
      else lastErr=`Gemini 호출 실패 (${m}): ${String(err&&err.message||err)}`;
  ...
  return {raw:String(raw).replace(/```[a-z]*|```/gi,"").trim(),model,api_calls:apiCalls};
}
function parseGeminiScreenshotPass_(raw,candidates,keyMap){
  const memberIds=new Set(),uncertain=[],notes=[],seenLines=new Set();let verify=false,parsedAny=false;
  const addUncertain=(t)=>{t=String(t||"").trim();if(t&&!seenLines.has(t)){seenLines.add(t);uncertain.push({text:t});}};
  for(const line0 of String(raw||"").split(/\r?\n/)){
    const line=String(line0||"").trim();if(!line)continue;
    const p=line.split("|").map(x=>x.trim()),kind=String(p[0]||"").toUpperCase();
    if(kind==="M"&&p.length>=3){const c=keyMap.get(p[1]);if(c){memberIds.add(c.member_id);parsedAny=true;}}
    else if(kind==="U"&&p.length>=2){const seen=p.slice(1).join("|").trim();if(seen){const m=screenshotMatchDetectedName_(seen,candidates);if(m.id)memberIds.add(m.id);else if(m.hint)addUncertain(`${seen} → 후보 ${m.hint}`);else addUncertain(`미등록/대상외: ${seen}`);parsedAny=true;verify=true;}}
    else if(kind==="Q"&&p.length>=2){const seen=String(p[1]||"").trim(),c=keyMap.get(String(p[2]||"").trim());addUncertain(c?`${seen||"판독불명"} → 후보 ${c.nickname}`:(seen||"판독불명"));parsedAny=true;verify=true;}
    else if(kind==="V"){verify=/^(1|Y|YES|TRUE|NEED)$/i.test(String(p[1]||""));if(p.length>=3&&p.slice(2).join("|").trim())notes.push(`verify:${p.slice(2).join("|").trim()}`);parsedAny=true;}
    else if(kind==="N"&&p.length>=2){notes.push(p.slice(1).join("|").trim());parsedAny=true;}
  }
  if(!parsedAny&&String(raw||"").includes("{")){
    let obj=null,clean=String(raw||"").trim();try{obj=JSON.parse(clean)}catch(_){const a=clean.indexOf("{"),b=clean.lastIndexOf("}");if(a>=0&&b>a){try{obj=JSON.parse(clean.slice(a,b+1).replace(/,\s*([}\]])/g,"$1"))}catch(__){}}}
    if(obj){
      for(const m of Array.isArray(obj.matches)?obj.matches:[]){const key=String(m&&m.candidate_key||m&&m.key||"").trim(),c=keyMap.get(key);if(c){memberIds.add(c.member_id);parsedAny=true;}else{const id=String(m&&m.member_id||"").trim();if(id&&candidates.some(x=>String(x.member_id)===id)){memberIds.add(id);parsedAny=true;}}}
      for(const name of Array.isArray(obj.observed_unregistered)?obj.observed_unregistered:[]){const seen=String(name||"").trim();if(!seen)continue;const m=screenshotMatchDetectedName_(seen,candidates);if(m.id)memberIds.add(m.id);else if(m.hint)addUncertain(`${seen} → 후보 ${m.hint}`);else addUncertain(`미등록/대상외: ${seen}`);parsedAny=true;verify=true;}
      for(const u of Array.isArray(obj.uncertain)?obj.uncertain:[]){const seen=String(u&&u.seen_text||"").trim(),key=String(u&&u.candidate_key||u&&u.key||"").trim(),c=keyMap.get(key);addUncertain(c?`${seen||"판독불명"} → 후보 ${c.nickname}`:(seen||"판독불명"));parsedAny=true;verify=true;}
      if(obj.verify===true||Number(obj.verify)===1)verify=true;if(obj.note)notes.push(String(obj.note).slice(0,300));
    }
  }
  if(!parsedAny)throw new Error(`Gemini 결과 해석 실패: ${String(raw||"").slice(0,260)}`);
  return {member_ids:[...memberIds],uncertain,notes,verify};
}
function geminiScreenshotPrompt_(roster,context,imageCount,pass,confirmedKeys=[]){
  const scope=String(context.scope||"WORLD")==="SERVER"?"서버보스":"월드보스",scopeText=String(context.scope||"WORLD")==="SERVER"?`대상 서버: ${String(context.server_name||context.server_id||"해당 서버")}`:"대상: 연합 전체";
  const second=pass===2;
  return `당신은 모바일 MMORPG 보스전 스크린샷의 참가자 닉네임을 판독하는 비전 검사기입니다.\n이번 요청에는 같은 보스 회차의 스크린샷 ${imageCount}장이 함께 들어 있습니다. 모든 이미지를 하나의 참가자 집합으로 통합하고 중복은 제거하세요. ${scope}. ${scopeText}.\n\n[후보 명단: 후보키 / 닉네임 / 서버 / 길드]\n${roster}\n${second?`\n[독립 2차 누락검증]\n1차에서 이미 확인된 후보키: ${confirmedKeys.join(", ")||"없음"}\n이 2차 검증은 1차 결과를 취소하거나 줄이는 단계가 아닙니다. 다른 확대 영역을 독립적으로 다시 보고 1차에서 놓친 추가 인원만 적극적으로 찾으세요. 이미 확인된 인원을 다시 출력해도 무방하지만, 최종 결과는 합집합으로 사용됩니다.\n`:""}\n매우 중요:\n- 후보 명단에 있다는 이유만으로 추측해서 넣지 마세요. 실제 픽셀에서 닉네임이 보여야 합니다.\n- ${imageCount}장을 서로 별개 참가자 목록으로 취급하지 말고 동일 회차의 보완 증거로 함께 보세요.\n- 답을 내기 전에 내부적으로 화면 전체/중앙/좌우/상하를 3차례 재검토하되 검토 과정은 출력하지 마세요.\n- 특정 길드만 집중하지 말고 후보 전체 길드를 동일하게 확인하세요. 길드명/태그는 닉네임 식별 보조 증거일 뿐 참가 제한이 아닙니다.\n- 동일/유사 닉네임을 길드 정보로도 구분할 수 없으면 M으로 확정하지 말고 Q로 남기세요.\n\n판독 규칙:\n1) 플레이어 캐릭터 닉네임만 판독합니다. 길드명, 직책, 보스명, 데미지 숫자, 채팅, 퀘스트, 스킬/UI 문구는 제외합니다.\n2) 후보와 명확히 일치하면 M|후보키|실제로읽힌닉네임\n3) 화면에는 보이지만 후보 명단에 없으면 U|실제로읽힌닉네임\n4) 비슷하지만 확정할 수 없으면 Q|실제로읽힌닉네임|가장가까운후보키\n5) 추가 독립 검증이 필요할 만큼 가려짐/군중밀집/흐림/미확정 글자가 남아 있으면 V|1|짧은이유, 충분히 확정적이면 V|0|ok\n6) 참고 메모는 N|짧은메모\n7) 설명문, 마크다운, JSON은 쓰지 말고 위 한 줄 형식만 출력하세요.`;
}
async function geminiAnalyzeScreenshotPass_(env,images,candidates,context,pass,deadline,confirmedKeys=[]){
  ...
  const bundle=await geminiScreenshotBundleViews_(env,images,pass),r=geminiScreenshotRoster_(candidates),prompt=geminiScreenshotPrompt_(r.roster,context,bundle.valid.length,pass,confirmedKeys),parts=[{text:prompt}];
  for(const v of bundle.views)parts.push({inlineData:{mimeType:String(v.mime_type||"image/jpeg"),data:String(v.data||"")}});
  const api=await geminiScreenshotApiCall_(env,parts,pass,deadline),parsed=parseGeminiScreenshotPass_(api.raw,candidates,r.keyMap);
  return {...parsed,model:api.model,api_calls:api.api_calls,view_count:bundle.views.length,image_count:bundle.valid.length,keyed:r.keyed};
}
async function geminiScreenshotMatch_(env,images,candidates,context={}){
  const valid=(Array.isArray(images)?images:[]).slice(0,4).filter(x=>x&&x.data&&String(x.mime_type||"").startsWith("image/"));if(!valid.length)throw new Error("분석할 이미지가 없습니다.");
  /* 524 방지를 위해 한 Worker 요청 전체에 시간 예산을 둔다. 보통 1차 1회, 필요할 때만 2차 1회. */
  const deadline=Date.now()+48000,pass1=await geminiAnalyzeScreenshotPass_(env,valid,candidates,context,1,deadline,[]),ids=new Set(pass1.member_ids||[]),uncertain=[...(pass1.uncertain||[])],notes=[...(pass1.notes||[])];
  let pass2=null,verificationNeeded=Boolean(pass1.verify||uncertain.length||ids.size===0),verificationSkipped=false;
  if(verificationNeeded&&deadline-Date.now()>12000){
    const roster=geminiScreenshotRoster_(candidates),confirmed=[...ids].map(id=>{const x=roster.keyed.find(k=>k.member_id===id);return x&&x.key}).filter(Boolean);
    try{pass2=await geminiAnalyzeScreenshotPass_(env,valid,candidates,context,2,deadline,confirmed);for(const id of pass2.member_ids||[])ids.add(id);for(const u of pass2.uncertain||[])uncertain.push(u);for(const n of pass2.notes||[])notes.push(n);}
    catch(e){notes.push(`2차검증실패:${String(e&&e.message||e).slice(0,180)}`);}
  }else if(verificationNeeded)verificationSkipped=true;
  const uniq=[],seen=new Set();for(const u of uncertain){const t=String(u&&u.text||"");if(t&&!seen.has(t)){seen.add(t);uniq.push({text:t});}}
  const calls=Number(pass1.api_calls||0)+Number(pass2&&pass2.api_calls||0),passes=pass2?2:1,model=`Gemini Free Vision · P1 ${pass1.model}${pass2?` + P2 ${pass2.model}`:""}`;
  return {model,member_ids:[...ids],uncertain:uniq,note:`api_calls=${Math.max(1,calls)} | external_passes=${passes} | images=${valid.length} | pass1_views=${pass1.view_count}${pass2?` | pass2_views=${pass2.view_count}`:""}${verificationNeeded?" | verify_requested=1":" | verify_requested=0"}${verificationSkipped?" | verify_skipped_budget=1":""}${notes.length?" | "+notes.join(" | ").slice(0,520):""}`};
}
async function analyzeScreenshotCore_(env,aid,eventId,images,meta={}){
  ...
  const sourceMessage=String(meta.source_message_id||"");if(sourceMessage){const old=await first(env.DB,"SELECT * FROM attendance_screenshot_checks WHERE alliance_id=? AND source_message_id=? AND status='done' ORDER BY created_at DESC LIMIT 1",aid,sourceMessage);if(old)return currentScreenshotCheck_(env,aid,old);}
  const candidates=await screenshotCandidates_(env,aid,e),srv=e.server_id?await serverMeta(env,aid,e.server_id):null,analysis=await geminiScreenshotMatch_(env,images,candidates,{scope:String(e.boss_scope||"WORLD"),server_id:String(e.server_id||""),server_name:String(srv?.server_name||"")}),byId=new Map(candidates.map(x=>[String(x.member_id),x]));
  /* 같은 보스 회차의 여러 스크린샷은 '한 번 놓치면 사라지는' 방식이 아니라 확인 근거를 누적한다. */
  const cumulativeIds=new Set(),prior=await all(env.DB,"SELECT recognized_json FROM attendance_screenshot_checks WHERE alliance_id=? AND event_id=? AND status='done' ORDER BY created_at ASC",aid,eid);
  for(const row of prior)for(const item of jsonArray_(row.recognized_json)){const id=String(item&&item.member_id||"");if(id&&byId.has(id))cumulativeIds.add(id);}
  const currentIds=[...new Set((analysis.member_ids||[]).map(String).filter(id=>byId.has(id)))];for(const id of currentIds)cumulativeIds.add(id);
  const recognized=[...cumulativeIds].map(id=>byId.get(id)).filter(Boolean);
  const attendance=await botAttendanceList_(env,aid,eid),attendedIds=new Set((attendance.participants||[]).map(x=>String(x.member_id||"")).filter(Boolean)),recognizedIds=new Set(recognized.map(x=>String(x.member_id||"")));
  const matched=recognized.filter(x=>attendedIds.has(String(x.member_id||""))),screenshotOnly=recognized.filter(x=>!attendedIds.has(String(x.member_id||"")));
  ...
  const checkId=uid("shot"),now=nowIso(),note=[String(analysis.note||""),`current=${currentIds.length}`,`cumulative=${recognized.length}`,`previous_checks=${prior.length}`].filter(Boolean).join(" | ");await run(env.DB,"INSERT INTO attendance_screenshot_checks(check_id,alliance_id,event_id,boss_id,boss_name,source,source_message_id,source_channel_id,result_message_id,image_count,recognized_count,recognized_json,matched_json,screenshot_only_json,attendance_only_json,uncertain_json,model,status,created_at,created_by,note) VALUES(?,?,?,?,?,?,?,?,?,?, ?,?,?,?,?,?,?, 'done',?,?,?)",checkId,aid,eid,e.boss_id||"",e.boss_name||"",String(meta.source||"web"),sourceMessage,String(meta.source_channel_id||""),"",Math.min(4,Array.isArray(images)?images.length:0),recognized.length,JSON.stringify(recognized),JSON.stringify(matched),JSON.stringify(screenshotOnly),JSON.stringify(attendanceOnly),JSON.stringify(analysis.uncertain),analysis.model,now,String(meta.created_by||""),note);
  for(const id of currentIds)await recordAttendanceEvidence_(env,aid,eid,id,"screenshot",`check ${checkId}`);
  /* 이미 출석된 사람도 스샷 확인 경로가 추가되므로 Discord 참여 메시지를 최신화한다. */
  await queueAttendanceSync_(env,aid,eid,"upsert","screenshot evidence");
  const current=await currentScreenshotCheck_(env,aid,await first(env.DB,"SELECT * FROM attendance_screenshot_checks WHERE check_id=?",checkId));
  if(meta.notify_kakao)await queueKakaoAttendanceLive_(env,aid,eid,current,"스크린샷 대조");
  return current;
}
...
async function deleteAttendanceScreenshotChecks(env,allianceId,eventId,authToken){
  ...
  await ensureAttendanceEvidenceSchema_(env);await run(env.DB,"DELETE FROM attendance_source_evidence WHERE alliance_id=? AND event_id=? AND source_key='screenshot'",aid,e.event_id);
  await queueAttendanceSync_(env,aid,e.event_id,"upsert","screenshot checks deleted");
...
async function deleteAttendanceEvent(env,allianceId,eventId,authToken){
  ...
  await Promise.all([
    ...
    run(env.DB,"DELETE FROM attendance_source_evidence WHERE alliance_id=? AND event_id=?",aid,eid),
...
async function analyzeAttendanceScreenshots(env,allianceId,eventId,images,authToken,notifyKakao=false){
  ...
  return analyzeScreenshotCore_(env,aid,e.event_id,images,{source:"web",created_by:"web",notify_kakao:boolVal(notifyKakao,false)});
}
async function finalizeAttendanceScreenshotBatch(env,allianceId,eventId,authToken){
  const aid=String(allianceId||""),eid=String(eventId||""),e=await first(env.DB,"SELECT * FROM boss_events WHERE alliance_id=? AND event_id=?",aid,eid);if(!e)throw new Error("보스 회차를 찾을 수 없습니다.");
  await requireBossAttendanceOperate(env,authToken,aid,e.boss_scope||"WORLD",e.server_id||"");
  const latest=await latestScreenshotCheckByEvent_(env,aid,eid);if(!latest)throw new Error("완료된 스크린샷 대조 결과가 없습니다.");
  await queueKakaoAttendanceLive_(env,aid,eid,latest,"스크린샷 대조 완료");
  await queueAttendanceSync_(env,aid,eid,"upsert","screenshot batch finalized");
  return latest;
}
...
async function botScreenshotAnalyze_(env,aid,p){return analyzeScreenshotCore_(env,aid,p.event_id,Array.isArray(p.images)?p.images:[],{source:"discord",source_message_id:p.source_message_id||"",source_channel_id:p.source_channel_id||"",created_by:p.discord_user_id||"",notify_kakao:true});}
...
async function setAllianceBossAttendanceEnabled(env,allianceId,bossId,enabled,authToken){
  const aid=String(allianceId||""),bid=String(bossId||""),b=await first(env.DB,"SELECT * FROM bosses WHERE alliance_id=? AND boss_id=? AND enabled=1",aid,bid);
  if(!b)throw new Error("보스를 찾을 수 없습니다.");
  await requireBossConfigScope(env,authToken,aid,b.boss_scope||"WORLD",b.server_id||"");
  const on=bint(enabled,false),now=nowIso();
  await run(env.DB,"UPDATE bosses SET attendance_enabled=?,updated_at=? WHERE alliance_id=? AND boss_id=?",on,now,aid,bid);
  const open=!on?await first(env.DB,"SELECT event_id FROM boss_events WHERE alliance_id=? AND boss_id=? AND attendance_status='open' LIMIT 1",aid,bid):null;
  return {ok:true,boss_id:bid,boss_name:b.boss_name,attendance_enabled:Boolean(on),message:`${b.boss_name} · 컷 시 출석 ${on?"ON":"OFF"}${open?" · 현재 진행 중인 출석은 유지됩니다.":""}`};
}
...
async function attendanceSummary(env,event){const data=await botAttendanceList_(env,String(event.alliance_id||""),event.event_id),people=data.participants||[],names=people.map(attendanceParticipantLine_).filter(Boolean),manual=String(event.event_type||"")==="manual_participation";return `📋 ${event.boss_name} ${manual?"참여체크":"출석"} 종료\n참여 ${names.length}명${names.length?"\n"+names.join(" · "):""}`;}
...
const BOSS_ALERT_LATE_GRACE_MS = 90 * 1000;
function bossAlertId_(boss,spawn,type){
  return `alert_${String(boss.alliance_id)}_${String(boss.boss_id)}_${Math.floor(spawn.getTime()/1000)}_${String(type)}`;
}
async function generateDueBossAlerts_(env,now=new Date()){
  ...
  for(const b of bosses){
    let spawns=[];
    if(b.boss_type==="cooldown"){
      const st=await first(env.DB,"SELECT next_spawn_at FROM boss_state WHERE alliance_id=? AND boss_id=?",b.alliance_id,b.boss_id);
      if(st?.next_spawn_at)spawns=[new Date(st.next_spawn_at)];
    }else spawns=scheduledCandidates(b,now);
    for(const spawn of spawns){
      if(!spawn||Number.isNaN(spawn.getTime()))continue;
      if(b.boss_type!=="cooldown")await ensureScheduledAttendanceEvent(env,b,spawn,now);
      for(const a of [{type:"10m",mins:10,on:boolVal(b.alert_10m,true)},{type:"5m",mins:5,on:boolVal(b.alert_5m,true)},{type:"1m",mins:1,on:boolVal(b.alert_1m,true)},{type:"spawn",mins:0,on:boolVal(b.alert_spawn,true)}]){
        if(!a.on)continue;
        const due=new Date(spawn.getTime()-a.mins*60000),lag=now.getTime()-due.getTime();
        /* 미래 알림은 만들지 않고, 90초 이상 늦은 알림도 만들지 않는다. */
        if(lag<0||lag>BOSS_ALERT_LATE_GRACE_MS)continue;
        /* 젠이 이미 지난 뒤에는 10/5/1분 전 알림을 절대 만들지 않는다. */
        if(a.type!=="spawn"&&now.getTime()>=spawn.getTime())continue;
        const srv=b.server_id?await serverMeta(env,b.alliance_id,b.server_id):null,t=formatKst(spawn,false).slice(-5),msg=a.type==="spawn"?`🚨 [${b.boss_name}] 젠 시간입니다! (${t} 젠)`:`⏰ [${b.boss_name}] 젠 ${a.mins}분 전입니다. (${t} 젠)`;
        const alertId=bossAlertId_(b,spawn,a.type);
        await run(env.DB,"INSERT OR IGNORE INTO boss_alert_queue(alert_id,alliance_id,boss_id,boss_name,boss_scope,server_id,server_name,spawn_at,alert_type,due_at,status,discord_status,kakao_status,message,created_at,target,discord_channel_id,discord_channel_ids,note) VALUES(?,?,?,?,?,?,?,?,?,?,'queued','queued','queued',?,?,'alliance',?,'','')",alertId,b.alliance_id,b.boss_id,b.boss_name,b.boss_scope||"WORLD",b.server_id||"",srv?.server_name||"",spawn.toISOString(),a.type,due.toISOString(),msg,nowS,b.discord_alert_channel_id||"");
      }
    }
  }
}
...
async function processScheduled(env){
  ...
  await generateDueBossAlerts_(env,now);
...
const RPC = {
  ...
  saveGuildLoot, analyzeGuildLootScreenshot, saveGuildLootScreenshotLocal, importGuildLootScreenshot, markGuildLootSold, deleteGuildLoot, addFundTransaction, deleteFundTransaction, getGuildStats,
  getAttendanceEvents, getAttendanceRoster, setAttendanceMember, analyzeAttendanceScreenshots, finalizeAttendanceScreenshotBatch, applyAttendanceScreenshotMissing, deleteAttendanceScreenshotChecks, deleteAttendanceEvent,
  saveAllianceBoss, setAllianceBossAttendanceEnabled, disableAllianceBoss, clearAllianceBossState, registerAllianceBossCut, setAllianceBossSpawn, closeBossAttendance
};
...
async function botAttendanceSet_(env,aid,p){
  ...
  const requested=boolVal(p.attended,true),now=nowIso(),g=actor.guild,srv=await serverMeta(env,aid,g.server_id),old=await first(env.DB,"SELECT * FROM boss_attendance WHERE event_id=? AND member_id_at_event=? ORDER BY recorded_at DESC LIMIT 1",e.event_id,actor.member_id),src=botSource_(p),discordUserId=p.kakao_user_key?String(old?.discord_user_id||""):String(p.discord_user_id||old?.discord_user_id||"");
  if(old&&old.source)await recordAttendanceEvidence_(env,aid,e.event_id,actor.member_id,old.source,"legacy before bot update");
  if(requested){
    if(old)await run(env.DB,"UPDATE boss_attendance SET attended=1,recorded_at=?,cancelled_at='',source=?,discord_user_id=?,guild_id_at_event=?,guild_name_at_event=?,server_id_at_event=?,server_name_at_event=?,game_nickname_at_event=? WHERE attendance_id=?",now,src,discordUserId,actor.guild_id,g.guild_name,g.server_id||"",srv?.server_name||g.game_server_name||"",actor.nickname,old.attendance_id);else await run(env.DB,"INSERT INTO boss_attendance(attendance_id,alliance_id,event_id,boss_id,discord_user_id,guild_id_at_event,guild_name_at_event,server_id_at_event,server_name_at_event,member_id_at_event,game_nickname_at_event,attended,recorded_at,cancelled_at,online_status_snapshot,source,note) VALUES(?,?,?,?,?,?,?,?,?,?,?,1,?,'','',?,'')",uid("att"),aid,e.event_id,e.boss_id,discordUserId,actor.guild_id,g.guild_name,g.server_id||"",srv?.server_name||g.game_server_name||"",actor.member_id,actor.nickname,now,src);
    await recordAttendanceEvidence_(env,aid,e.event_id,actor.member_id,src,`${src} attendance`);
    await queueAttendanceSync_(env,aid,e.event_id,"upsert",src+" attendance");
    return {event_id:e.event_id,boss_id:e.boss_id,boss_name:e.boss_name,nickname:actor.nickname,attended:true,requested_attended:true,remaining_sources:[attendanceCanonicalSource_(src)].filter(Boolean)};
  }
  /* 취소는 요청한 채널의 출첵 근거만 제거한다. 다른 채널/스크린샷 근거가 남으면 참여 상태는 유지한다. */
  await removeAttendanceEvidence_(env,aid,e.event_id,actor.member_id,src);
  const remaining=await attendanceExplicitSourcesForMember_(env,aid,e.event_id,actor.member_id),keep=remaining.length>0,primary=keep?remaining[0]:src;
  if(old)await run(env.DB,"UPDATE boss_attendance SET attended=?,cancelled_at=?,source=?,discord_user_id=?,guild_id_at_event=?,guild_name_at_event=?,server_id_at_event=?,server_name_at_event=?,game_nickname_at_event=? WHERE attendance_id=?",keep?1:0,keep?"":now,primary,discordUserId,actor.guild_id,g.guild_name,g.server_id||"",srv?.server_name||g.game_server_name||"",actor.nickname,old.attendance_id);
  await queueAttendanceSync_(env,aid,e.event_id,"upsert",src+" attendance source removed");
  return {event_id:e.event_id,boss_id:e.boss_id,boss_name:e.boss_name,nickname:actor.nickname,attended:keep,requested_attended:false,remaining_sources:remaining,remaining_source_labels:remaining.map(attendanceSourceLabelKey_)};
}
...
async function botAttendanceList_(env,aid,eventId){
  ...
  const rows=await all(env.DB,"SELECT * FROM boss_attendance WHERE event_id=? AND attended=1 ORDER BY server_name_at_event,guild_name_at_event,game_nickname_at_event",e.event_id),evidence=await attendanceEvidenceMap_(env,aid,e.event_id);
  ...
  return {event_id:e.event_id,event_type:e.event_type||"cut",boss_id:e.boss_id,boss_name:e.boss_name,boss_scope:e.boss_scope||"WORLD",server_id:e.server_id||"",cut_at:formatKst(e.cut_at,false),next_spawn_at:e.next_spawn_at?formatKst(e.next_spawn_at,false):"",attendance_status:e.attendance_status,discord_message_id:e.discord_message_id||"",discord_channel_id:e.discord_channel_id||"",discord_message_links:links,attendance_channel_id:channelIds[0]||"",attendance_channel_ids:channelIds.join(","),participants:rows.map(x=>{const src=attendanceSourcesFor_(evidence,x.member_id_at_event,x.source||"");return {attendance_id:x.attendance_id,discord_user_id:x.discord_user_id||"",guild_id:x.guild_id_at_event||"",guild_name:x.guild_name_at_event||"",server_id:x.server_id_at_event||"",server_name:x.server_name_at_event||"",member_id:x.member_id_at_event||"",nickname:x.game_nickname_at_event||"",source:x.source||"",sources:src.sources,source_labels:src.source_labels,source_label:src.source_label};})};
}
...
async function botPullAlerts_(env,aid,p){
  /* 소비자가 폴링하는 순간에도 현재 시각 기준 알림을 생성한다.
     ROOT cron -> 소비자 cron 두 단계 지연을 줄여 10/5/1분 알림을 제시간에 가깝게 보낸다. */
  const current=new Date();
  await generateDueBossAlerts_(env,current);
  const target=String(p.target||"discord").toLowerCase()==="kakao"?"kakao":"discord",key=target+"_status",limit=Math.max(1,Math.min(100,Number(p.limit||20))),now=current.getTime(),rows=await all(env.DB,`SELECT * FROM boss_alert_queue WHERE alliance_id=? AND ${key}='queued' ORDER BY due_at,created_at LIMIT 200`,aid),live=[];
  const seen=new Set();
  for(const x of rows){
    const due=new Date(x.due_at),spawn=new Date(x.spawn_at),type=String(x.alert_type||"");
    if(Number.isNaN(due.getTime())){await run(env.DB,`UPDATE boss_alert_queue SET ${key}='expired' WHERE alert_id=?`,x.alert_id);continue;}
    /* 예정 시각 전 큐는 아직 보내지 않는다. */
    if(due.getTime()>now)continue;
    const late=now-due.getTime();
    /* 90초 이상 늦은 알림은 잘못된 시각에 보내지 않고 폐기한다. */
    if(late>BOSS_ALERT_LATE_GRACE_MS){await run(env.DB,`UPDATE boss_alert_queue SET ${key}='expired' WHERE alert_id=?`,x.alert_id);continue;}
    /* 10/5/1분 전 알림은 보스 젠 이후에는 절대 보내지 않는다. */
    if(type!=="spawn"&&!Number.isNaN(spawn.getTime())&&now>=spawn.getTime()){
      await run(env.DB,`UPDATE boss_alert_queue SET ${key}='expired' WHERE alert_id=?`,x.alert_id);continue;
    }
    /* 젠 알림도 너무 늦으면 위 grace에서 제거된다. */
    const dedupe=`${String(x.boss_id)}|${String(x.spawn_at)}|${type}`;
    if(seen.has(dedupe)){
      await run(env.DB,`UPDATE boss_alert_queue SET ${key}='expired' WHERE alert_id=?`,x.alert_id);continue;
    }
    seen.add(dedupe);
    /* 과거 버전에서 랜덤 alert_id로 생성된 동일 알림까지 함께 정리한다. */
    await run(env.DB,`UPDATE boss_alert_queue SET ${key}='expired' WHERE alliance_id=? AND boss_id=? AND spawn_at=? AND alert_type=? AND alert_id<>? AND ${key}='queued'`,aid,x.boss_id,x.spawn_at,type,x.alert_id);
...
async function botKakaoCommand_(env,aid,p){
  ...
  if(cmd==="보스"||cmd==="보스확인"||cmd==="보탐"){const list=await botBosses_(env,aid,{kakao_user_key:key});if(!list.length)return {reply:"등록된 활성 보스가 없습니다."};const groups={};for(const b of list){const k=b.boss_scope==="WORLD"?"월드":b.server_name;(groups[k]||(groups[k]=[])).push(b);}const out=[];const keys=Object.keys(groups).sort((a,b)=>a==="월드"?1:(b==="월드"?-1:a.localeCompare(b,"ko",{numeric:true})));for(const k of keys){out.push(`[${k}]`);const rows=groups[k].sort((a,b)=>{const av=String(a.next_spawn_at||"9999-12-31 23:59"),bv=String(b.next_spawn_at||"9999-12-31 23:59");const c=av.localeCompare(bv);return c||String(a.boss_name||"").localeCompare(String(b.boss_name||""),"ko");});for(const b of rows)out.push(`${b.boss_name} · 컷 ${b.last_kill_at?b.last_kill_at.slice(-5):"-"} · 예정 ${b.next_spawn_at?b.next_spawn_at.slice(-5):"-"}`);}return {reply:out.join("\n")};}
  ...
  if(cmd==="참여"||cmd==="취소"){const target=args.join(" ").trim(),e=await botOpenEvent_(env,aid,target,{kakao_user_key:key}),r=await botAttendanceSet_(env,aid,{kakao_user_key:key,event_id:e.event_id,attended:cmd==="참여"});if(cmd==="취소"&&r.attended){const ls=Array.isArray(r.remaining_source_labels)?r.remaining_source_labels:[];return {reply:`↩️ Kakao 출첵 취소 · ${r.boss_name} · ${r.nickname}\n✅ 다른 확인 경로${ls.length?` [${ls.join("+")}]`:""}가 남아 실제 참여는 유지됩니다.`,event_id:r.event_id||"",sync_attendance:true};}return {reply:`${cmd==="참여"?"✅ 참여":"↩️ 취소"} · ${r.boss_name} · ${r.nickname}`,event_id:r.event_id||"",sync_attendance:true};}
  ...
  if(cmd==="보탐대조"){const r=await botScreenshotLatest_(env,aid,{kakao_user_key:key,boss:args.join(" ")});if(r.empty)return {reply:`📸 ${r.boss_name} · 저장된 스크린샷 대조 결과가 없습니다.`};const names=function(rows){return (rows||[]).map(x=>{const n=x.nickname||x.text||"";const ls=Array.isArray(x.source_labels)?x.source_labels:[];return n+(ls.length?` [${ls.join("+")}]`:"")}).filter(Boolean).join(" · ")||"-"};const grouped=function(rows){const g={};for(const x of rows||[]){const k=(x.server_name?x.server_name+" / ":"")+(x.guild_name||"기타");(g[k]||(g[k]=[])).push(x)}return Object.keys(g).sort((a,b)=>a.localeCompare(b,"ko")).map(k=>`[${k}] ${names(g[k])}`).join("\n")||"-"};return {reply:`📸 ${r.boss_name} 보탐 대조
${r.server_name?`대상 ${r.server_name} · 길드별 구분`:"월드 · 서버/길드별 구분"}
누적 스샷 확인 ${Number(r.recognized_count||0)}명 · 분석 ${Number(r.check_count||1)}회
✅ 스샷 확인+출석 ${(r.matched||[]).length}명
${grouped(r.matched)}
➕ 스샷 확인+미출석 ${(r.screenshot_only||[]).length}명
${grouped(r.screenshot_only)}
🔎 출석됨·회차 스샷 누적 미확인 ${(r.attendance_only||[]).length}명
${grouped(r.attendance_only)}
❓ 이번 분석 후보/불확실 ${(r.uncertain||[]).length}명 · ${names(r.uncertain)}`};}
