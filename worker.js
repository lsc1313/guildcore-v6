...
async function geminiScreenshotBundleViews_(env,images,pass=1){
  ...
  for(let i=0;i<valid.length;i++){
    const mode=pass===2?"pass2":(multi?"pass1":"full"),views=await geminiScreenshotViews_(env,valid[i],mode,multi);
...
async function geminiScreenshotApiCall_(env,parts,pass,deadline){
  ...
  const base=pass===1?[30000,15000,9000]:[22000,11000,7000];
...
async function geminiScreenshotMatch_(env,images,candidates,context={}){
  ...
  const deadline=Date.now()+70000,pass1=await geminiAnalyzeScreenshotPass_(env,valid,candidates,context,1,deadline,[]),ids=new Set(pass1.member_ids||[]),uncertain=[...(pass1.uncertain||[])],notes=[...(pass1.notes||[])];
