...
async function moondreamAnalyzeView_(env,view,candidates,context={}){
  const model="@cf/moondream/moondream3.1-9B-A2B";
  const scope=String(context.scope||"WORLD")==="SERVER"?"서버보스":"월드보스";
  const scopeText=String(context.scope||"WORLD")==="SERVER"?`대상은 ${String(context.server_name||context.server_id||"해당 서버")}에 속한 연합 전체 길드원이다.`:"대상은 연합 전체 길드원이다.";
  const question=`모바일 MMORPG 보스전 스크린샷의 확대 조각이다. ${scope}. ${scopeText}
화면에 보이는 플레이어 캐릭터 닉네임을 OCR로 읽어라. 특히 초록/노랑 길드명이나 직책 글자 아래쪽에 있는 파랑/청록/흰색의 짧은 캐릭터 이름 라벨을 우선 읽어라.
길드명, 보스명, 데미지 숫자, 채팅, 퀘스트, 스킬명, UI 글자는 제외한다. 후보 명단을 보고 추측하지 말고 실제 화면에서 읽힌 글자만 출력한다.
확실히 읽힌 닉네임은 줄마다 NAME|읽힌닉네임, 애매한 것은 UNCERTAIN|읽힌글자 형식으로만 출력하고 설명은 쓰지 마라.`;
  let result;
  try{result=await env.AI.run(model,{task:"query",image:`data:${String(view.mime_type||"image/png")};base64,${String(view.data||"")}`,question,reasoning:false,temperature:0,max_tokens:420,stream:false});}
  catch(e){throw new Error(`Moondream OCR 실패(${view.label}): ${e&&e.message?e.message:String(e)}`);}
  const raw=moondreamText_(result).trim(),ids=[],uncertain=[],detected=[];
  for(const line of raw.split(/\r?\n/)){
    let m=line.match(/^\s*(?:NAME|MATCH)\s*\|\s*(.+?)\s*$/i);
    if(m&&m[1]){const name=String(m[1]).trim();if(name)detected.push(name);continue;}
    m=line.match(/^\s*UNCERTAIN\s*\|\s*(.+?)\s*$/i);if(m&&m[1]){const name=String(m[1]).trim();if(name)uncertain.push(name);}
  }
  /* 모델이 형식을 어겨도 짧은 줄은 닉네임 후보로 한 번 더 본다. */
  if(!detected.length){for(const line of raw.split(/\r?\n/)){const t=String(line||"").replace(/^[-*•\d.\s]+/,"").trim();if(t&&t.length<=20&&!/[=:]/.test(t))detected.push(t)}}
  for(const name of detected){const m=screenshotMatchDetectedName_(name,candidates);if(m.id)ids.push(m.id);else if(m.hint)uncertain.push(`${name} → 후보 ${m.hint}`);else uncertain.push(name);}
  return {ids:[...new Set(ids)],uncertain:[...new Set(uncertain)],detected:[...new Set(detected)],note:raw.slice(0,520)};
}
async function workersAiScreenshotMatch_(env,images,candidates,context={}){
  ...
  /* 한 요청에서 최대 2장만 무거운 OCR. 여러 장은 Discord에서 각각 답장 업로드해도 같은 회차 결과로 누적 가능. */
  ...
  const memberIds=new Set(),uncertain=new Set(),notes=[`scope=${String(context.scope||"WORLD")} server=${String(context.server_name||context.server_id||"-")} candidates=${candidates.length}`];
  for(let i=0;i<valid.length;i++){
    const plan=await screenshotVisionPlan_(env,valid[i]);notes.push(`image ${i+1}: ${plan.regions.length} lean tiles`);
    /* 핵심: 타일 전체를 base64 배열로 쌓지 않는다. 2개씩 변환→OCR→즉시 폐기하여 1102 CPU/메모리 초과를 방지한다. */
    for(let pos=0;pos<plan.regions.length;pos+=2){
      const regs=plan.regions.slice(pos,pos+2);
      const transformed=await Promise.allSettled(regs.map(r=>screenshotTransform_(env,plan.bytes,plan.mime,r,r.label,plan.w,plan.h)));
      const ready=[];
      for(let j=0;j<transformed.length;j++){
        const t=transformed[j];if(t.status==="fulfilled")ready.push(t.value);else notes.push(`${i+1}/${regs[j].label}: ${String(t.reason&&t.reason.message?t.reason.message:t.reason).slice(0,140)}`);
      }
      const scanned=await Promise.allSettled(ready.map(v=>moondreamAnalyzeView_(env,v,candidates,context)));
      for(let j=0;j<scanned.length;j++){
        const s=scanned[j],v=ready[j];
        if(s.status==="fulfilled"){
          for(const id of s.value.ids)memberIds.add(id);
          for(const t of s.value.uncertain){const q=String(t||"").trim();if(q)uncertain.add(q);}
          if(s.value.note)notes.push(`${i+1}/${v.label}: ${s.value.note.slice(0,120)}`);
        }else notes.push(`${i+1}/${v.label}: ${String(s.reason&&s.reason.message?s.reason.message:s.reason).slice(0,140)}`);
      }
    }
    /* 1차 6타일에서 전혀 못 읽은 경우에만 중앙을 더 작게 한 번 재확대. Gemma 4중 호출은 제거. */
    if(!memberIds.size){
      const center={label:"center-x4",top:.25,left:.30,width:.40,height:.50};
      try{
        const v=await screenshotTransform_(env,plan.bytes,plan.mime,center,center.label,plan.w,plan.h,1536);
        const o=await moondreamAnalyzeView_(env,v,candidates,context);
        for(const id of o.ids)memberIds.add(id);for(const t of o.uncertain){const q=String(t||"").trim();if(q)uncertain.add(q);}
        if(o.note)notes.push(`${i+1}/center-x4: ${o.note.slice(0,140)}`);
      }catch(e){notes.push(`${i+1}/center-x4: ${String(e&&e.message?e.message:e).slice(0,140)}`);}
    }
  }
  return {model:"Workers AI OCR / Moondream 3.1 server-roster tiles",member_ids:[...memberIds],uncertain:[...uncertain].map(text=>({text})),note:notes.join(" | ")};
}
async function analyzeScreenshotCore_(env,aid,eventId,images,meta={}){
  ...
  const candidates=await screenshotCandidates_(env,aid,e),srv=e.server_id?await serverMeta(env,aid,e.server_id):null,analysis=await workersAiScreenshotMatch_(env,images,candidates,{scope:String(e.boss_scope||"WORLD"),server_id:String(e.server_id||""),server_name:String(srv?.server_name||"")}),byId=new Map(candidates.map(x=>[x.member_id,x])),recognized=analysis.member_ids.map(id=>byId.get(id)).filter(Boolean);
