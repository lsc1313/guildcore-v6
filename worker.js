...
async function screenshotTransform_(env,bytes,mime,region,label){
  ...
  const sourceW=Math.max(1,Number(info&&info.width||0)),sourceH=Math.max(1,Number(info&&info.height||0));
  const cropW=Math.max(1,Math.round(sourceW*Math.max(.08,Number(region.width||1))));
  /* 게임 닉네임 OCR은 생성형 업스케일이 글자 획을 바꿀 수 있으므로 bicubic 보간 + 선명도/대비 보강만 사용 */
  const targetW=Math.min(2304,Math.max(1536,Math.round(cropW*3.2)));
  const response=(await env.IMAGES.input(screenshotImageStream_(bytes,mime))
    ...
    .transform({width:targetW,fit:"scale-up",upscale:"interpolate",sharpen:4,contrast:1.35,brightness:1.04,saturation:.88})
  ...
  return {label,data:bytesBase64_(out),mime_type:"image/png",note:`${label} ${sourceW}x${sourceH} crop ${(region.width*100).toFixed(0)}%x${(region.height*100).toFixed(0)}% → ${targetW}w OCR`};
}
async function screenshotVisionViews_(env,img){
  ...
  const regions=[];
  if(landscape){
    /* 폰 게임 스샷 기준: 화면을 4x3의 작은 겹침 타일로 쪼개 닉네임 픽셀을 3배 이상 키운다. */
    const xs=[0,.22,.44,.66],ys=[0,.30,.60];
    for(let yi=0;yi<ys.length;yi++)for(let xi=0;xi<xs.length;xi++)regions.push({label:`r${yi+1}c${xi+1}`,top:ys[yi],left:xs[xi],width:.34,height:.40});
  }else{
    const xs=[0,.30,.60],ys=[0,.22,.44,.66];
    for(let yi=0;yi<ys.length;yi++)for(let xi=0;xi<xs.length;xi++)regions.push({label:`r${yi+1}c${xi+1}`,top:ys[yi],left:xs[xi],width:.40,height:.34});
  }
  const out=[],failed=[];
...
function moondreamText_(result){
  if(result==null)return "";
  if(typeof result==="string")return result;
  if(typeof result.answer==="string")return result.answer;
  if(typeof result.response==="string")return result.response;
  if(typeof result.caption==="string")return result.caption;
  if(result.result){if(typeof result.result.answer==="string")return result.result.answer;if(typeof result.result.response==="string")return result.result.response;}
  return workersAiText_(result);
}
function screenshotNorm_(v){try{return String(v||"").normalize("NFKC").replace(/\s+/g,"").toLowerCase()}catch{return String(v||"").replace(/\s+/g,"").toLowerCase()}}
async function moondreamAnalyzeView_(env,view,candidates){
  const model="@cf/moondream/moondream3.1-9B-A2B";
  const candidateLines=candidates.map(x=>`${x.member_id}\t${x.nickname}`).join("\n");
  const question=`이 이미지는 모바일 MMORPG 보스전 화면의 확대 조각입니다. 캐릭터 머리 위나 캐릭터 주변에 떠 있는 플레이어 닉네임 글자를 OCR로 아주 세밀하게 읽으세요.\n\n아래 등록 후보 중 이 이미지에서 실제로 글자가 보이는 닉네임만 정확한 후보 철자로 출력하세요. 보이지 않으면 절대 추측하지 마세요. 데미지 숫자, 보스명, 길드명, 스킬명, UI 문구는 제외하세요.\n\n출력 형식은 줄마다 MATCH|닉네임 입니다. 애매하지만 후보와 비슷한 글자는 UNCERTAIN|읽힌글자 로 출력할 수 있습니다. 다른 설명은 쓰지 마세요.\n\n[등록 후보]\n${candidateLines}`;
  let result;
  try{result=await env.AI.run(model,{task:"query",image:`data:${String(view.mime_type||"image/png")};base64,${String(view.data||"")}`,question,reasoning:false,temperature:0,max_tokens:700});}
  catch(e){throw new Error(`Moondream OCR 실패(${view.label}): ${e&&e.message?e.message:String(e)}`);}
  const raw=moondreamText_(result).trim(),normRaw=screenshotNorm_(raw),ids=[],uncertain=[];
  for(const c of candidates){const n=screenshotNorm_(c.nickname);if(n&&normRaw.includes(n))ids.push(c.member_id);}
  for(const line of raw.split(/\r?\n/)){
    const m=line.match(/^\s*UNCERTAIN\s*\|\s*(.+?)\s*$/i);if(m&&m[1])uncertain.push(String(m[1]).trim());
  }
  return {ids:[...new Set(ids)],uncertain:[...new Set(uncertain)],note:raw.slice(0,500)};
}
async function gemmaFallbackAnalyzeView_(env,view,rosterJson,schema,candidates){
  const model="@cf/google/gemma-4-26b-a4b-it";
  const prompt=`모바일 MMORPG 스크린샷의 확대 조각입니다. 캐릭터 주변의 플레이어 닉네임만 읽고 등록 후보와 대조하세요. 보이는 경우에만 member_id를 matched_member_ids에 넣고 절대 추측하지 마세요.\n[등록 후보]\n${rosterJson}`;
  const req={messages:[{role:"system",content:"게임 화면의 작은 한글 닉네임 OCR 분석기입니다."},{role:"user",content:prompt}],image:`data:${String(view.mime_type||"image/png")};base64,${String(view.data||"")}`,temperature:0,max_completion_tokens:700,chat_template_kwargs:{enable_thinking:false}};
  const result=await env.AI.run(model,{...req,response_format:{type:"json_schema",json_schema:schema}});
  const out=normalizeWorkersAiMatch_(workersAiJson_(result)),allowed=new Set(candidates.map(x=>x.member_id));
  return {ids:out.matched_member_ids.filter(id=>allowed.has(id)),uncertain:out.uncertain_text,note:out.note};
}
async function workersAiScreenshotMatch_(env,images,candidates){
  ...
  const roster=candidates.map(x=>({member_id:x.member_id,nickname:x.nickname,guild:x.guild_name,server:x.server_name})),rosterJson=JSON.stringify(roster);
  ...
  const memberIds=new Set(),uncertain=new Set(),notes=[];
  for(let i=0;i<valid.length;i++){
    const views=await screenshotVisionViews_(env,valid[i]);notes.push(`image ${i+1}: ${views.length} microtiles`);
    /* Worker/AI 과부하를 피하면서도 빠르게 처리하도록 4개씩 병렬 OCR */
    for(let pos=0;pos<views.length;pos+=4){
      const batch=views.slice(pos,pos+4),settled=await Promise.allSettled(batch.map(v=>moondreamAnalyzeView_(env,v,candidates)));
      for(let j=0;j<settled.length;j++){
        const s=settled[j],v=batch[j];
        if(s.status==="fulfilled"){
          for(const id of s.value.ids)memberIds.add(id);for(const t of s.value.uncertain){const q=String(t||"").trim();if(q)uncertain.add(q);}
          if(s.value.note)notes.push(`${i+1}/${v.label}: ${s.value.note.slice(0,160)}`);
        }else notes.push(`${i+1}/${v.label}: ${String(s.reason&&s.reason.message?s.reason.message:s.reason).slice(0,180)}`);
      }
    }
    /* OCR 특화 모델이 한 명도 못 찾은 경우 중앙 4타일을 Gemma에 한 번 더 교차검증 */
    if(!memberIds.size){
      const fallbackViews=views.slice(Math.max(0,Math.floor((views.length-4)/2)),Math.max(0,Math.floor((views.length-4)/2))+4);
      const settled=await Promise.allSettled(fallbackViews.map(v=>gemmaFallbackAnalyzeView_(env,v,rosterJson,schema,candidates)));
      for(let j=0;j<settled.length;j++)if(settled[j].status==="fulfilled"){
        const o=settled[j].value;for(const id of o.ids)memberIds.add(id);for(const t of o.uncertain){const q=String(t||"").trim();if(q)uncertain.add(q);}
      }
    }
  }
  return {model:"Workers AI OCR / Moondream 3.1 microtile + Gemma fallback",member_ids:[...memberIds],uncertain:[...uncertain].map(text=>({text})),note:notes.join(" | ")};
}
