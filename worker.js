...
async function analyzeGuildLootScreenshot(env,guildId,img={},authToken){
  ...
  if(!env.AI)throw new Error("Cloudflare Workers AI 바인딩(AI)이 설정되지 않았습니다. wrangler.jsonc의 ai.binding=AI를 확인하세요.");
  const data=String(img&&img.data||"").trim(),mime=String(img&&img.mime_type||"image/jpeg").toLowerCase();
  if(!data)throw new Error("이미지 데이터가 없습니다.");if(!/^image\/(jpeg|jpg|png|webp)$/.test(mime))throw new Error("JPG, PNG, WEBP 이미지만 사용할 수 있습니다.");
  const bytes=base64Bytes_(data);if(bytes.length>8*1024*1024)throw new Error("이미지는 8MB 이하로 올려주세요.");
  const hash=await sha256BytesHex_(bytes),gid=String(guildId);
  const prev=await first(env.DB,"SELECT * FROM loot_screenshot_imports WHERE guild_id=? AND image_hash=?",gid,hash);
  if(prev){let items=[];try{items=JSON.parse(prev.items_json||"[]")}catch{}return {ok:true,analysis_id:prev.analysis_id,items,duplicate:true,imported_at:prev.imported_at||"",model:prev.model||"Cloudflare Moondream"};}
  const question=`이 이미지는 MMORPG 게임의 아이템 획득 시스템 로그 스크린샷입니다.\n각 획득 문장은 보통 '[색상 아이템명]을 획득하였습니다.' 또는 '[색상 아이템명]를 획득하였습니다.' 형태입니다.\n아이템명 글자색으로 등급을 판정하세요: 파란색/청색=희귀, 빨간색=영웅, 노란색/금색=전설.\n흰색 설명문, 시각, 채팅, 일반 시스템 문구는 무시하세요.\n아이템명에 '(귀속)' 또는 '귀속'이 붙으면 bound=1, 아니면 0입니다. 귀속도 읽되 나중에 등록 제외할 수 있게 표시하세요.\n아이템명은 화면에 적힌 그대로 옮기고 임의로 고치거나 줄이지 마세요.\n반드시 획득 문장 하나당 아래 형식 한 줄만 출력하세요. 설명문이나 마크다운은 쓰지 마세요.\nI|희귀|0|아이템명\nI|영웅|1|아이템명 (귀속)\nI|전설|0|아이템명\n등급을 확실히 판단할 수 없는 줄은 출력하지 마세요.`;
  let result;
  try{result=await env.AI.run("@cf/moondream/moondream3.1-9B-A2B",{task:"query",image:`data:${mime};base64,${data}`,question,reasoning:false,temperature:0,max_tokens:2200,stream:false});}
  catch(e){throw new Error("Cloudflare 아이템 OCR 실패: "+errorMessage(e));}
  const raw=lootVisionAnswer_(result),items=parseLootVision_(raw),analysisId=uid("lootshot"),now=nowIso();
  await run(env.DB,"INSERT INTO loot_screenshot_imports(analysis_id,guild_id,image_hash,items_json,raw_text,model,created_at,imported_at,source) VALUES(?,?,?,?,?,?,?,'','cloudflare')",analysisId,gid,hash,JSON.stringify(items),raw.slice(0,6000),"@cf/moondream/moondream3.1-9B-A2B",now);
  return {ok:true,analysis_id:analysisId,items,duplicate:false,imported_at:"",model:"Cloudflare Moondream 3.1"};
}
async function importGuildLootScreenshot(env,guildId,analysisId,indexes=[],authToken){
  await requireGuildOperate(env,authToken,guildId);await ensureLootScreenshotSchema_(env);
  ...
  const pick=new Set((Array.isArray(indexes)?indexes:[]).map(Number).filter(Number.isFinite)),groups=new Map();
  for(const x of items){const ix=Number(x.index),rarity=normalizeLootRarity_(x.rarity),name=String(x.item_name||"").trim(),bound=Boolean(x.bound)||name.includes("귀속");if(!pick.has(ix)||!name||!rarity||bound)continue;const key=rarity+"\u0000"+name;const g=groups.get(key)||{item_name:name,rarity,quantity:0};g.quantity++;groups.set(key,g);}
  if(!groups.size)throw new Error("등록할 수 있는 아이템이 선택되지 않았습니다.");
  const now=nowIso();for(const g of groups.values())await run(env.DB,"INSERT INTO loot(loot_id,guild_id,raid_id,item_name,quantity,status,winner_member_id,sale_amount,fund_rate,fund_amount,guild_support_amount,distribution_pool,completed_at,note,created_at,source,rarity) VALUES(?,?,?,?,?,'보유','',NULL,NULL,NULL,NULL,NULL,'','',?,'screenshot_cloudflare',?)",uid("loot"),gid,"",g.item_name,g.quantity,now,g.rarity);
...
const RPC = {
  ...
  saveGuildLoot, analyzeGuildLootScreenshot, importGuildLootScreenshot, markGuildLootSold, deleteGuildLoot, addFundTransaction, deleteFundTransaction, getGuildStats,
