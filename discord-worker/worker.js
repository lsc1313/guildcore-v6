...
async function deleteChannelMessage(env,channelId,messageId){
  const r=await fetch(`https://discord.com/api/v10/channels/${channelId}/messages/${messageId}`,{method:"DELETE",headers:botHeaders(env,false)});
  if(!r.ok&&r.status!==404)throw new Error(`Discord 메시지 삭제 실패 ${r.status}: ${await r.text()}`);
  return true;
}
async function flushDiscordMessageDeletes(env,discordGuildId){
  let pulled;try{pulled=await apiCall(env,discordGuildId,"discord_message_deletes_pull",{limit:40});}catch(e){console.log("discord delete pull error",discordGuildId,e.message);return;}
  const done=[];
  for(const x of (pulled.messages||[])){
    try{await deleteChannelMessage(env,String(x.channel_id||""),String(x.message_id||""));done.push(String(x.delete_id));}
    catch(e){console.log("discord message delete error",x.delete_id,e.message)}
  }
  if(done.length)try{await apiCall(env,discordGuildId,"discord_message_deletes_mark",{delete_ids:done,status:"sent"});}catch(e){console.log("discord delete mark error",e.message)}
}
...
async function runDiscordCron(env){
  ...
  for(const discordGuildId of ids){
    ...
    await Promise.all([flushAlerts(env,discordGuildId),flushAttendanceResults(env,discordGuildId),flushAttendanceSync(env,discordGuildId),flushDiscordMessageDeletes(env,discordGuildId)]);
